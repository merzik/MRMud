...
// Changed to give color to effects on EQ - Nodd - 08/04
{
	...
	|| obj->poison!=NULL )
	{
		...
		if (IS_OBJ_STAT(obj, ITEM_INVIS))										/* BLUE */
			blen = str_apd_max(buf, " \033[34m\033[1m(Invis)" ,    blen, MAX_STRING_LENGTH);

		if (IS_AFFECTED(ch, AFF_DETECT_EVIL) && IS_OBJ_STAT(obj, ITEM_EVIL))	/* RED */
			blen = str_apd_max(buf, " \033[31m\033[22m(Red Aura)" , blen, MAX_STRING_LENGTH);

		if (IS_AFFECTED(ch, AFF_DETECT_MAGIC) && IS_OBJ_STAT(obj, ITEM_MAGIC))	/* WHITE */
			blen = str_apd_max(buf, " \033[37m\033[1m(Magical)" ,  blen, MAX_STRING_LENGTH);

		if (IS_OBJ_STAT(obj, ITEM_GLOW))										/* YELLOW */
			blen = str_apd_max(buf, " \033[33m\033[1m(Glowing)" ,  blen, MAX_STRING_LENGTH);

		if (IS_OBJ_STAT(obj, ITEM_HUM))											/* PINK */
			blen = str_apd_max(buf, " \033[31m\033[1m(Humming)" ,  blen, MAX_STRING_LENGTH);

		if (IS_OBJ_STAT(obj, ITEM_PROTOTYPE))									/* PURPLE */
			blen = str_apd_max(buf, " \033[35m\033[1m(PROTO)" ,    blen, MAX_STRING_LENGTH);

		if ((IS_NPC(ch)||number_percent()<ch->pcdata->learned[gsn_detect_forgery])
			&& IS_OBJ_STAT(obj, ITEM_FORGERY))									/* GREY */
			blen = str_apd_max(buf, " \033[37m\033[22m(Forgery)" ,  blen, MAX_STRING_LENGTH);
			...
			obj->poison->owner==ch->pcdata->pvnum )
		{
			...
			if( obj->poison->for_npc )
				sprintf( buf2, " \033[32m\033[1m(Poisoned M%c)", obj->poison->poison_type );	/*ANSI_GREEN_BOLD */
			else
				sprintf( buf2, " \033[32m\033[1m(Poisoned C%c)", obj->poison->poison_type );	/* ANSI_GREEN_BOLD */
			blen = str_apd_max( buf, buf2, blen, MAX_STRING_LENGTH);
		}
	}
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	*buf = '\0';
	...
	if ( victim != ch
		...
		(which_god(ch) == GOD_POLICE) || (ch->level >= MAX_LEVEL-2)))
	{
...
void do_delete( CHAR_DATA *ch, char *arg)
{
	...
	if( !PassRqd || check_password(ch, arg) )
	{
		/*
		strcpy( buf3, ch->name);
		buf3[1]='\0';
		...
		*/
		...
		strcpy(buf, get_player_filename(ch->name, PFILE_TYPE_NORMAL));
		strcpy(buf2, get_player_filename(ch->name, PFILE_TYPE_DELETE));
		/*
		if( COMPRESS_FILES)
		{
...
/*  Score changed 01/04 to include color and new layout - Nodd */
void get_string_score_v1( CHAR_DATA *ch, CHAR_DATA *viewer)
{
	...
	*get_string_score_txt = '\0';
	...
	char catColor[13], valColor[13];
	strcpy( catColor, ANSI_GREEN_BOLD);
	strcpy( valColor, ANSI_CYAN_BOLD);
	...
	/*****|Nodd. Ahh you're an MBA...let me show you how.                        |****/
	sprintf(buf1,"%s%s",ch->name,IS_NPC(ch) ? " the monster" : ch->pcdata->title);
	if(strlen(buf1)>70)
		buf1[70]='\0';
	sprintf(buf,"%s   | %s%s%s|\n\r", ANSI_YELLOW_DIM, ANSI_YELLOW_BOLD, str_resize(buf1,tbuf1,-69),ANSI_YELLOW_DIM);
	...
	/*****|      Class:Cleric       Level:95  Reinc:1x  RACE: Elf     |*****/
	sprintf(buf,"   %s|     %sClass: %s%s%s   Level: %s%-3d%s   Reinc: %s%1dx%s   Race: %s%s%s    |\n\r", 
	        ANSI_YELLOW_DIM, catColor, valColor, str_resize(class_table[ch->class].who_name_long,tbuf1,-12), 
	        catColor, valColor, ch->level, 
	        catColor, valColor, ch->pcdata->reincarnation, 
	        catColor, valColor, str_resize(race_table[ch->race].race_name,buf2,-8), ANSI_YELLOW_DIM);
	leng = str_apd_max( get_string_score_txt, buf, leng, MAX_STRING_LENGTH);
	/*********************************************************************************/
	
	/*****   |   [Ill: 0 Ele: 0 Rog: 0 Ran: 0 Nec: 0 Cle:95 Asn: 0 Mon: 0]|*/
	sprintf(buf,"%s   | %s   %s%s:%s%2d%s  %s:%s%2d%s  %s:%s%2d%s  %s:%s%2d%s  %s:%s%2d%s  %s:%s%2d%s  %s:%s%2d%s  %s:%s%2d%s %s   |\n\r", 
		ANSI_YELLOW_DIM,catColor,ANSI_BLUE_BOLD,
		/* Code below: Only classes used will be bold cyan */
		class_table[0].who_name,(ch->mclass[0] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, ch->mclass[0],ANSI_BLUE_BOLD,
		class_table[1].who_name,(ch->mclass[1] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, ch->mclass[1],ANSI_BLUE_BOLD,
		class_table[2].who_name,(ch->mclass[2] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, ch->mclass[2],ANSI_BLUE_BOLD,
		class_table[3].who_name,(ch->mclass[3] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, ch->mclass[3],ANSI_BLUE_BOLD,
		class_table[4].who_name,(ch->mclass[4] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, ch->mclass[4],ANSI_BLUE_BOLD,
		class_table[5].who_name,(ch->mclass[5] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, ch->mclass[5],ANSI_BLUE_BOLD,
		class_table[6].who_name,(ch->mclass[6] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, ch->mclass[6],ANSI_BLUE_BOLD,
		class_table[7].who_name,(ch->mclass[7] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, ch->mclass[7],catColor, ANSI_YELLOW_DIM);
	leng = str_apd_max( get_string_score_txt, buf, leng, MAX_STRING_LENGTH);
	/*********************************************************************************/
	...
	/*****   |            Stats: Str:28  Int:33  Wis:14  Dex:18  Con:16             |*/
	sprintf(buf,"   %s|          %sStats: %sStr: %s%2d  %sInt: %s%2d  %sWis: %s%2d  %sDex: %s%2d  %sCon: %s%2d          %s|\n\r",
		ANSI_YELLOW_DIM,catColor,ANSI_BLUE_BOLD,ANSI_CYAN_BOLD,
		get_curr_str(ch),ANSI_BLUE_BOLD, ANSI_CYAN_BOLD,
		get_curr_int(ch),ANSI_BLUE_BOLD, ANSI_CYAN_BOLD,
		get_curr_wis(ch),ANSI_BLUE_BOLD, ANSI_CYAN_BOLD,
		get_curr_dex(ch),ANSI_BLUE_BOLD, ANSI_CYAN_BOLD,
		get_curr_con(ch),ANSI_YELLOW_DIM);
	leng = str_apd_max( get_string_score_txt, buf, leng, MAX_STRING_LENGTH);
	/*********************************************************************************/
	...
	/*****      |      Hitpoints: 1594/1594                    Mana: 203/203           |*/
	sprintf(buf1,"%d/%d",ch->hit,ch->max_hit);   // sets up hitpoint info
	sprintf(buf2,"%d/%d",ch->mana,ch->max_mana); // sets up mana info
	sprintf(buf,"   %s|       %sHitpoints: %s%s                   %sMana: %s%s     %s|\n\r", 
		  ANSI_YELLOW_DIM, 
		  catColor, valColor, str_resize(buf1,tbuf1,-11), 
		  catColor, valColor, str_resize(buf2,tbuf2,-11),ANSI_YELLOW_DIM);
	leng = str_apd_max( get_string_score_txt, buf, leng, MAX_STRING_LENGTH);
	/*********************************************************************************/
	
	/*****   |       Movement: 1122/1122               Practices: 227          |*/
	sprintf(buf1,"%d/%d",ch->move,ch->max_move);
	sprintf(buf,"   %s|        %sMovement: %s%s              %sPractices: %s%-3d             %s|\n\r", 
		ANSI_YELLOW_DIM, 
	        catColor, valColor, str_resize(buf1,tbuf1,-11),
 		catColor, valColor, ch->practice, ANSI_YELLOW_DIM);
	leng = str_apd_max( get_string_score_txt, buf, leng, MAX_STRING_LENGTH);
	/*********************************************************************************/
	
   	/* |        DamRoll: 2                         Hitroll: 1                            |	*/
	if(ch->level>14) // only if level > 14
	{ // determine dam roll
		OBJ_DATA *wield;
		...
		if((!IS_NPC(ch))&&(ch->mclass[CLASS_MONK]>0)&&(wield==NULL))
		{
			...
			sprintf(buf2,"%dd%d+%d",numDice,sizeDice,sizePlus);
		}
		else
			sprintf(buf2,"%-3d",GET_DAMROLL(ch));
		
		sprintf(buf,"   %s|        %sDam Roll: %s%s%s               Hit Roll: %s%3d%s             |\n\r",
	        	ANSI_YELLOW_DIM, 
	        	catColor, valColor, str_resize(buf2,tbuf2,-11), 
	        	catColor, valColor, GET_HITROLL(ch), ANSI_YELLOW_DIM);
		leng = str_apd_max( get_string_score_txt, buf, leng, MAX_STRING_LENGTH);
	}
	/*********************************************************************************/

	/*   |       Carrying: 3/1000 items         Carry Weight: 15/1000000        |	*/
	sprintf(buf2,"%d/%d Items",ch->carry_number,can_carry_n(ch));
	sprintf(buf1,"%d/%d",ch->carry_weight,can_carry_w(ch));
	sprintf(buf,"%s   |        %sCarrying: %s%s%s       Carry Weight: %s%s%s      |\n\r", 
	        ANSI_YELLOW_DIM, 
	        catColor, valColor, str_resize(buf2,tbuf2,-15), 
	        catColor, valColor, str_resize(buf1,tbuf1,-10), ANSI_YELLOW_DIM);
	leng = str_apd_max( get_string_score_txt, buf, leng, MAX_STRING_LENGTH);
	/*********************************************************************************/
	...
	/*****   |    Speed:running       Armor Class:WORSE than naked!(-160)           |*/
	...
	isDark = 0;  // Used to tally the spaces needed for formatting, dark colors use an extra escape code
	gac = GET_AC(ch)+ch->level*3;
	/* Code below: Armor Class statement changes color with tier - Nodd 01/04 */
	if ( gac >=  101 )     { strcpy(buf2, ANSI_RED_DIM);    strcat( buf2, "WORSE than naked!");  isDark=1;}
	else if ( gac >=   80 ){ strcpy(buf2, ANSI_RED_BOLD);    strcat( buf2, "Naked");}
	else if ( gac >=   60 ){ strcpy(buf2, ANSI_MAGENTA_DIM); strcat( buf2, "Wearing clothes");   isDark=1;}
	else if ( gac >=   40 ){ strcpy(buf2, ANSI_MAGENTA_BOLD); strcat( buf2, "Slightly armored");}
	else if ( gac >=   20 ){ strcpy(buf2, ANSI_CYAN_DIM);   strcat( buf2, "Somewhat armored");  isDark=1;}
	else if ( gac >=    0 ){ strcpy(buf2, ANSI_WHITE_BOLD);  strcat( buf2, "Armored");}
	else if ( gac >= - 20 ){ strcpy(buf2, ANSI_CYAN_DIM);   strcat( buf2, "Well armored");      isDark=1;}
	else if ( gac >= - 40 ){ strcpy(buf2, ANSI_BLUE_DIM);   strcat( buf2, "Strongly armored");  isDark=1;}
	else if ( gac >= - 60 ){ strcpy(buf2, ANSI_BLUE_BOLD);   strcat( buf2, "Heavily armored");}
	else if ( gac >= - 80 ){ strcpy(buf2, ANSI_YELLOW_DIM); strcat( buf2, "Superbly armored");  isDark=1;}
	else if ( gac >= -100 ){ strcpy(buf2, ANSI_YELLOW_BOLD); strcat( buf2, "Divinely armored");}
	                  else { strcpy(buf2, ANSI_WHITE_BOLD);  strcat( buf2, "Invincible!");}

	/* level > 24 sees ac number */
	if(ch->level>24)
      {
		sprintf(buf,"%s (%2d)", valColor, GET_AC(ch));
		strcat(buf2,buf);
		

		sprintf(buf,"%s   |%s           Speed: %s%s%s       Armor Class: %s%s%s |\n\r",
		ANSI_YELLOW_DIM,
		catColor, valColor, str_resize(buf1,tbuf1,-8), 
		catColor, valColor, str_resize(buf2,tbuf2,(-41)), ANSI_YELLOW_DIM);
      }
	else /* level < 24, no number */
	{
		sprintf(buf,"%s   |%s           Speed: %s%s%s       Armor Class: %s%s%s |\n\r",
		ANSI_YELLOW_DIM,
		catColor, valColor, str_resize(buf1,tbuf1,-8), 
		catColor, valColor, str_resize(buf2,tbuf2,-32), ANSI_YELLOW_DIM);
	}		

	leng = str_apd_max( get_string_score_txt, buf, leng, MAX_STRING_LENGTH);
	/*********************************************************************************/
	
	/*****   |        Wimpy:1300 hp         Alignment:angelic.(1000)                    |*/
	/* Code below: Alignment statement changes color with tier. - Nodd 01/04 */

	isDark = 0;
	if ( ch->alignment >  900 )      { strcpy(buf2, ANSI_WHITE_BOLD); strcat(   buf2, "Angelic");}
	else if ( ch->alignment >  700 ) { strcpy(buf2, ANSI_YELLOW_BOLD); strcat( buf2, "Saintly");}
	else if ( ch->alignment >  350 ) { strcpy(buf2, ANSI_YELLOW_DIM); strcat( buf2, "Good");    isDark = 1;}
	else if ( ch->alignment >  100 ) { strcpy(buf2, ANSI_BLUE_BOLD); strcat(   buf2, "Kind");}
	else if ( ch->alignment > -100 ) { strcpy(buf2, ANSI_WHITE_DIM); strcat(   buf2, "Neutral");isDark = 1;}
	else if ( ch->alignment > -350 ) { strcpy(buf2, ANSI_MAGENTA_BOLD); strcat( buf2, "Mean");}
	else if ( ch->alignment > -700 ) { strcpy(buf2, ANSI_MAGENTA_DIM); strcat( buf2, "Evil");    isDark = 1;}
	else if ( ch->alignment > -900 ) { strcpy(buf2, ANSI_RED_BOLD); strcat(    buf2, "Demonic");}
	                            else { strcpy(buf2, ANSI_RED_DIM); strcat(    buf2, "Satanic"); isDark = 1;}
	/* level > 17 sees alignment number */
	if(ch->level>17)
	{
		sprintf(buf,"%s (%d)", valColor, ch->alignment);
		strcat(buf2,buf);
		sprintf(buf1,"%d hp",ch->wimpy);
		sprintf(buf,"%s   | %s          Wimpy: %s%s%s         Alignment: %s%s%s          |\n\r",
		ANSI_YELLOW_DIM, 
		catColor, valColor, str_resize(buf1,tbuf1,-8), 
		catColor, valColor, str_resize(buf2,tbuf2,(-32)), ANSI_YELLOW_DIM);
	}
	else /* level < 17, no number */
	{
		sprintf(buf1,"%d hp",ch->wimpy);
		sprintf(buf,"%s   | %s          Wimpy: %s%s%s         Alignment: %s%s%s          |\n\r",
		ANSI_YELLOW_DIM, 
		catColor, valColor, str_resize(buf1,tbuf1,-8), 
		catColor, valColor, str_resize(buf2,tbuf2,(-23)),ANSI_YELLOW_DIM);

	}
	leng = str_apd_max( get_string_score_txt, buf, leng, MAX_STRING_LENGTH);
	/*********************************************************************************/
	
	/*****   |           AutoLoot:no            AutoSac:no                                |*/
	sprintf(buf,"%s   |%s        AutoLoot: %s%s%s                AutoSac: %s%s%s                     |\n\r",
	        ANSI_YELLOW_DIM,
	        catColor, valColor, (!IS_NPC(ch) && IS_SET(ch->act, PLR_AUTOLOOT)) ? "Yes" : "No ", 
	        catColor, valColor, (!IS_NPC(ch) && IS_SET(ch->act, PLR_AUTOSAC) ) ? "Yes" : "No ", ANSI_YELLOW_DIM);
	leng = str_apd_max( get_string_score_txt, buf, leng, MAX_STRING_LENGTH);
	/*********************************************************************************/
	
	/*****   |          MDeaths:15          Save Vs Spell:-12                          |*/
	sprintf(buf,"%s   |%s         MDeaths: %s%-4d         %sSave Vs Spell: %s%-4d%s                    |\n\r",
		ANSI_YELLOW_DIM, 
		catColor, valColor, ch->pcdata->history[HISTORY_KILL_BY_NPC], 
		catColor, valColor, GET_SAVING_THROW( ch ), ANSI_YELLOW_DIM );
	leng = str_apd_max( get_string_score_txt, buf, leng, MAX_STRING_LENGTH);
	/*********************************************************************************/
	...
	/*****   | Recall Room:Patrokles Avenue.             Death Room:Patrokles Avenue.                 |*/
	strcpy(buf2,get_room_index(ch->recall)->name);
	strcpy(buf1,get_room_index(ch->pcdata->death_room)->name);
	sprintf(buf,"%s   |%s Recall Room: %s%s%s Death Room: %s%s%s |\n\r",
		ANSI_YELLOW_DIM,
		catColor, valColor, str_resize(buf2,tbuf2,-21),
		catColor, valColor, str_resize(buf1,tbuf1,-21), ANSI_YELLOW_DIM);
	leng = str_apd_max( get_string_score_txt, buf, leng, MAX_STRING_LENGTH);
	/*********************************************************************************/
	...
	/*****   |Known: Hum Hal Elf Dro Dwa Gno Orc Ogr Avi Cen Git Tsa Tro            |*/
	buf2[0]='\0';
	for(cnt=0;cnt<MAX_RACE;cnt++)
	{
	        if(IS_SHIFT( ch->language, cnt))
	...
	sprintf(buf,"%s   | %sKnown: %s%s%s|\n\r", ANSI_YELLOW_DIM, catColor, valColor, str_resize(buf2,tbuf1,-62), ANSI_YELLOW_DIM);
	leng = str_apd_max( get_string_score_txt, buf, leng, MAX_STRING_LENGTH);
	/*********************************************************************************/
	
	/*****   |Current: Centaur                                                     |*/
	sprintf( buf, "%s   | %sCurrent: %s%s%s          |\n\r", 
		ANSI_YELLOW_DIM,
		catColor, valColor, str_resize(race_table[UNSHIFT(ch->speak)].race_name,tbuf1,-50), ANSI_YELLOW_DIM);
	leng = str_apd_max( get_string_score_txt, buf, leng, MAX_STRING_LENGTH);
	/*********************************************************************************/
...
//**************************************************************************************************	
//******************************* FIND A SPOT FOR HOURS!!! **********************************
//**************************************************************************************************
/*	sprintf(buf,"%s   |%s          Practices: %s%-3d%s                      Age:%s%3d%s yrs (%05d hrs) |\n\r",
			ANSI_YELLOW_DIM, ANSI_GREEN_BOLD,
			ANSI_CYAN_BOLD, ch->practice, ANSI_GREEN_BOLD,
			ANSI_CYAN_BOLD, get_age(ch), ANSI_GREEN_BOLD, (ch->played)/3600 + (IS_NPC(ch)?0:ch->pcdata->previous_hours));
	send_to_char( buf, ch);
*/
//**************************************************************************************************
//**************************************************************************************************
...
void do_status( CHAR_DATA *ch, char *argument )
{
	...
	if( strcasecmp( argument, "reincarnate") )
	{
		...
		sprintf(buf, "   %s|%s%s%s|\n\r", ANSI_YELLOW_DIM, ANSI_YELLOW_BOLD, str_resize(buf1,tbuf1, -70),ANSI_YELLOW_DIM);
		send_to_char( buf, ch); 
    }
	...
	sprintf(buf,"%s   | %s         Stats:  %sStr: %s%2d%s  Int: %s%2d%s  Wis: %s%2d%s  Dex: %s%2d%s  Con: %s%2d         %s|\n\r",
			ANSI_YELLOW_DIM, ANSI_GREEN_BOLD,  ANSI_BLUE_BOLD, ANSI_CYAN_BOLD,
			get_curr_str(ch), ANSI_BLUE_BOLD, ANSI_CYAN_BOLD,
			get_curr_int(ch), ANSI_BLUE_BOLD, ANSI_CYAN_BOLD,
			get_curr_wis(ch), ANSI_BLUE_BOLD, ANSI_CYAN_BOLD,
			get_curr_dex(ch), ANSI_BLUE_BOLD, ANSI_CYAN_BOLD,
    ...
    if( !strcasecmp( argument, "reincarnate") )		
	{
		float avgStat = (get_curr_str(ch)+get_curr_int(ch)+get_curr_wis(ch)+get_curr_dex(ch)+get_curr_con(ch))/5;
		
		sprintf(buf,"%s   |       %sAverage Stat: %s%4.1f                     %sSex: %s%s%s|\n\r",
				ANSI_YELLOW_DIM, ANSI_GREEN_BOLD, ANSI_CYAN_BOLD,avgStat, ANSI_GREEN_BOLD, ANSI_CYAN_BOLD,str_resize(((ch->sex)==1 ? "Male" : "Female"), tbuf1,-19),ANSI_YELLOW_DIM);
		send_to_char( buf, ch);
	}	
	...
	{
		sprintf(buf1,"%d/%d",ch->carry_weight,can_carry_w(ch));
		sprintf(buf,"%s   |%s   Practices: %s%-3d%s                      Carry Weight: %s%s    %s|\n\r",
				ANSI_YELLOW_DIM, ANSI_GREEN_BOLD, ANSI_CYAN_BOLD, ch->practice, ANSI_GREEN_BOLD, ANSI_CYAN_BOLD, str_resize(buf1,tbuf1,-13),ANSI_YELLOW_DIM);
		send_to_char( buf, ch);
	}
	...
	if( strcasecmp( argument, "reincarnate") )
	{
		...
		gac = GET_AC(ch)+ch->level*3;
		/* Code below: Armor Class statement changes color with tier - Nodd 01/04 */
		if ( gac >=  101 )     { strcpy(buf1, ANSI_RED_DIM);    strcat( buf1, "WORSE than naked!"); isDark = 1;}
		else if ( gac >=   80 ){ strcpy(buf1, ANSI_RED_BOLD);    strcat( buf1, "Naked");}
		else if ( gac >=   60 ){ strcpy(buf1, ANSI_MAGENTA_DIM); strcat( buf1, "Wearing clothes");  isDark = 1;}
		else if ( gac >=   40 ){ strcpy(buf1, ANSI_MAGENTA_BOLD); strcat( buf1, "Slightly armored");}
		else if ( gac >=   20 ){ strcpy(buf1, ANSI_CYAN_DIM);   strcat( buf1, "Somewhat armored"); isDark = 1;}
		else if ( gac >=    0 ){ strcpy(buf1, ANSI_WHITE_BOLD);  strcat( buf1, "Armored");}
		else if ( gac >= - 20 ){ strcpy(buf1, ANSI_CYAN_DIM);   strcat( buf1, "Well armored");     isDark = 1;}
		else if ( gac >= - 40 ){ strcpy(buf1, ANSI_BLUE_DIM);   strcat( buf1, "Strongly armored"); isDark = 1;}
		else if ( gac >= - 60 ){ strcpy(buf1, ANSI_BLUE_BOLD);   strcat( buf1, "Heavily armored");}
		else if ( gac >= - 80 ){ strcpy(buf1, ANSI_YELLOW_DIM); strcat( buf1, "Superbly armored"); isDark = 1;}
		else if ( gac >= -100 ){ strcpy(buf1, ANSI_YELLOW_BOLD); strcat( buf1, "Divinely armored");}
						  else { strcpy(buf1, ANSI_WHITE_BOLD);  strcat( buf1, "Invincible!");}

		if(ch->level>24)
		{
			sprintf(buf,"%s (%2d)", ANSI_CYAN_BOLD, GET_AC(ch));
			strcat(buf1,buf);
		}

		isDark2 = 0;
		if ( ch->alignment >  900 )
			{ strcpy(buf2, ANSI_WHITE_BOLD);  strcat( buf2, "Angelic");}
		else if ( ch->alignment >  700 )
			{ strcpy(buf2, ANSI_YELLOW_BOLD); strcat( buf2, "Saintly");}
		else if ( ch->alignment >  350 )
			{ strcpy(buf2, ANSI_YELLOW_DIM); strcat( buf2, "Good");    isDark2 = 1;}
		else if ( ch->alignment >  100 )
			{ strcpy(buf2, ANSI_BLUE_BOLD);   strcat( buf2, "Kind");}
		else if ( ch->alignment > -100 )
			{ strcpy(buf2, ANSI_WHITE_DIM);  strcat( buf2, "Neutral"); isDark2 = 1;}
		else if ( ch->alignment > -350 )
			{ strcpy(buf2, ANSI_MAGENTA_BOLD); strcat( buf2, "Mean");}
		else if ( ch->alignment > -700 )
			{ strcpy(buf2, ANSI_MAGENTA_DIM); strcat( buf2, "Evil");    isDark2 = 1;}
		else if ( ch->alignment > -900 )
			{ strcpy(buf2, ANSI_RED_BOLD);    strcat( buf2, "Demonic");}
		else
			{ strcpy(buf2, ANSI_RED_DIM);    strcat( buf2, "Satanic"); isDark2 = 1;}

		if(ch->level>17)
		{
			sprintf(buf,"%s (%d)", ANSI_CYAN_BOLD, ch->alignment);
			strcat(buf2,buf);
		}

		sprintf(buf, "%s   |%s Armor Class: %s%s  Alignment: %s %s|\n\r",
				ANSI_YELLOW_DIM, ANSI_GREEN_BOLD, str_resize(buf1,tbuf1, ch->level > 24 ? (-44) : (-35)), 
				ANSI_GREEN_BOLD, str_resize(buf2, tbuf2, ch->level > 17 ? (-34) : (-25)), ANSI_YELLOW_DIM);
	...
	if(ch->level>14)
	{
		...
		sprintf(buf,"%s   |%s     HitRoll: %s%-3d%s                           DamRoll: %s%s      %s|\n\r",
				ANSI_YELLOW_DIM, ANSI_GREEN_BOLD, ANSI_CYAN_BOLD, GET_HITROLL(ch), ANSI_GREEN_BOLD, ANSI_CYAN_BOLD ,str_resize(buf1,tbuf1,-11),ANSI_YELLOW_DIM);
		if( strcasecmp( argument, "reincarnate") )
			send_to_char( buf, ch);
	}
	...
	buf[63]='\0';
	sprintf(buf2,"%s   |%s       OTHER: %s%s%s|\n\r", ANSI_YELLOW_DIM, ANSI_GREEN_BOLD, ANSI_CYAN_BOLD, str_resize(buf,tbuf1,-56),ANSI_YELLOW_DIM);

	if( strcasecmp( argument, "reincarnate") )
		send_to_char( buf2, ch);

	sprintf(buf,"%s   |%s              Class: %s%s%sRace: %s%s%s|\n\r",
		ANSI_YELLOW_DIM, ANSI_GREEN_BOLD,
		ANSI_CYAN_BOLD, str_resize(class_table[ch->class].who_name_long,tbuf1,-24), ANSI_GREEN_BOLD,
		ANSI_CYAN_BOLD, str_resize(race_table[ch->race].race_name,tbuf2,-19),ANSI_YELLOW_DIM );
	...
	sprintf(buf,"%s   |%s              Known: %s%s%s|\n\r", ANSI_YELLOW_DIM, ANSI_GREEN_BOLD, ANSI_CYAN_BOLD,str_resize(buf2,tbuf1,-49), ANSI_YELLOW_DIM);
	if( !strcasecmp( argument, "reincarnate") )
		send_to_char( buf, ch);
	...
	if (IS_IMMORTAL(ch))
	{
		...
		ch_printf(ch, "%s   |%s   Immortal Data:  %sWizinvis [%s]                                       %s|\n\r",
				...
				IS_SET(ch->act, PLR_WIZINVIS) ? "X" : " ", ANSI_YELLOW_DIM);
		
		ch_printf(ch, "%s   |%s          Bamfin:%s  %s %s%s|\n\r", 
				ANSI_YELLOW_DIM, ANSI_MAGENTA_BOLD, ANSI_CYAN_BOLD, ch->name, 
				str_resize(buf1,tbuf1,-45), ANSI_YELLOW_DIM);
        
		ch_printf(ch, "%s   |%s         Bamfout:%s  %s %s%s|\n\r", 
				ANSI_YELLOW_DIM, ANSI_MAGENTA_BOLD, ANSI_CYAN_BOLD, ch->name, 
				str_resize(buf2,tbuf1,-45), ANSI_YELLOW_DIM);

		if (ch->pcdata->area)
		{
			ch_printf(ch, "%s   |%sVnums:   Room%s (%-5.5d - %-5.5d)   %sObject%s (%-5.5d - %-5.5d)   %sMob%s (%-5.5d - %-5.5d)|\n\r",
				ANSI_YELLOW_DIM, ANSI_GREEN_BOLD, ANSI_CYAN_BOLD,
				ch->pcdata->area->low_r_vnum, ch->pcdata->area->hi_r_vnum,
				ANSI_GREEN_BOLD, ANSI_CYAN_BOLD,
...
void get_spy_score( CHAR_DATA *ch, CHAR_DATA *viewer)
{
	char buf[MAX_STRING_LENGTH],buf1[MAX_STRING_LENGTH],buf2[MAX_STRING_LENGTH];
	char tbuf1[MAX_INPUT_LENGTH],tbuf2[MAX_INPUT_LENGTH];
	/* AFFECT_DATA *paf;*/
	...
	*get_spy_score_txt = '\0';
	...
	leng = str_cpy_max( get_spy_score_txt, buf, MAX_STRING_LENGTH);
	sprintf(buf,"%s-|__________________________________________________________________________|-\n\r",bold);
	...
	else if ( gac >= 40 ) strcpy( buf2, "wearing clothes.");
	else if ( gac >= 20 ) strcpy( buf2, "slightly armored.");
	else if ( gac >= 0 ) strcpy( buf2, "somewhat armored.");
	else if ( gac >= - 20 ) strcpy( buf2, "armored.");
	else if ( gac >= - 60 ) strcpy( buf2, "well armored.");
	else if ( gac >= -100 ) strcpy( buf2, "strongly armored.");
	else if ( gac >= -150 ) strcpy( buf2, "heavily armored.");
	else if ( gac >= -220 ) strcpy( buf2, "superbly armored.");
	else if ( gac >= -300 ) strcpy( buf2, "divinely armored.");
	...
	leng = str_apd_max( get_spy_score_txt, buf2, leng, MAX_STRING_LENGTH);
						...
						int outbufleng )
{
	...
	sprintf(buf,"   %s|%s AFFECTS:                                                             %s|\n\r", ANSI_YELLOW_DIM, ANSI_GREEN_BOLD, ANSI_YELLOW_DIM);
	oleng = str_apd_max( outbuf, buf, oleng, outbufleng );
	...
	if ( ch->first_affect != NULL )
	{
		...
		{
			sprintf( buf, "'%s%s%s'%s", ANSI_WHITE_BOLD, capitalize(skill_table[paf->type].name), ANSI_YELLOW_DIM, ANSI_GREEN_DIM );
			slen = strlen( skill_table[paf->type].name ) + 3;
			if ( ch->level >= 30 )
			{
				sprintf( buf2, " modifies %s by %d for %s%d %shours",
					affect_loc_name( paf->location ), paf->modifier, paf->duration>5?ANSI_GREEN_DIM:(paf->duration>1?ANSI_RED_DIM:ANSI_RED_BOLD),paf->duration,ANSI_GREEN_DIM );
				if(paf->duration <= 1)
					slen = slen + 1;
				slen += strlen( buf2 );
				strcat(buf,buf2);
			...
			if (ch->level >= 30)
			{
				for( *buf2='\0'; slen < 87; slen++)
					strcat( buf2, " " );
			}
			else
			{
				for( *buf2='\0'; slen < 67; slen++)
					strcat( buf2, " ");
			}			

			sprintf(buf1,"   %s|  %s%s%s|\n\r", ANSI_YELLOW_DIM, buf, buf2, ANSI_YELLOW_DIM);
			oleng = str_apd_max( outbuf, buf1, oleng, outbufleng );
		}
	}
	else
	{
		sprintf( buf, "   %s| %sNone.                                                                %s|\n\r", ANSI_YELLOW_DIM, ANSI_WHITE_DIM, ANSI_YELLOW_DIM);
		oleng = str_apd_max( outbuf, buf, oleng, outbufleng );
	}
	sprintf(buf,"   ========================================================================\n\r");
	oleng = str_apd_max( outbuf, buf, oleng, outbufleng );
	...
	*/
	...
	for( fpl=first_player; fpl!=NULL; fpl=fpl->next)
	{
		...
		*/
		...
		case MAX_LEVEL - 1: class = "MAS"; break;
...
{
	char buf[MAX_STRING_LENGTH],buf_race[20],buf_lang[100], buf_class[100];
	...
	int nMatch;
	int nTotal;
	bool fClass, fRace, fArea, fClanarmy;
	bool fClan, fOrder;
	...
	*/
	...
	for ( ;; )
	{
		...
		if( ( fArea = !strcasecmp( arg, "area" ) ) == TRUE )
		{
			...
			if( argument != NULL && *argument != '\0' )
				for( ptArea = first_area; ptArea != NULL; ptArea = ptArea->next )
				{
					strcpy( abuf, ptArea->name );
					if( !str_prefix( argument, abuf ) )
					{
						pArea = ptArea;
						break;
					}
		...
		fClanarmy = !strcasecmp( arg, "army" );

		if( (!strcasecmp( arg, "all")) || target || follower || enemy || fClan
			|| fRace || fOrder || fClass || fArea || fClanarmy )
		{
			lrang=0;
			hrang=100;
			arg[0]='\0';
		...
		else
		{
			...
			*/
			...
			else
			{
				...
				hrang=100;
				if( !enemy && !follower )
					for( fpl=first_player; fpl!=NULL && !fNameRestrict; fpl=fpl->next )
					{
						fch = fpl->ch;
						if ( is_name( arg, fch->name ) )
						{
							if( IS_SET( fch->act, PLR_WIZINVIS) && !IS_SET( ch->act, PLR_HOLYLIGHT ) )
							{
								send_to_char( "No one has that name.\n\r", ch);
								return;
							}
	...
	*/
	...
	for( fpl=first_player; fpl!=NULL; fpl=fpl->next)
	{
		...
		*/
		if( IS_SET( wch->act, PLR_WIZINVIS) && !IS_SET( ch->act, PLR_HOLYLIGHT ))
			continue;

		if ( iLevelUpper > 99 && ch->level < 99 )
			iLevelUpper = 99;
		...
		if( wch->level < iLevelLower
			...
			|| ( wch->level < lrang)
			|| ( wch->level > hrang)
			|| ( fArea && wch->in_room->area != pArea )
			|| ( fClanarmy && wch->pcdata->clan_army != 1 )
			|| ( fRace && wch->race != iRaceClass )
			|| ( fClass && wch->class != iRaceClass )
			|| ( (fClan || fOrder) && !IS_NPC(wch) && wch->pcdata->clan != iClan )
			|| ( follower && ((which_god( ch) != which_god( wch))))
			|| ( target && (get_attack_type(ch, wch) < KILL_MOB || wch==ch))
			|| ( enemy && ((which_god(ch)==which_god(wch)) ||
		...
		/* If this is a who area, don't show stealthed or hidden chars in the area without proper credentials */
		if ( fArea && wch != ch && ( is_affected( wch, gsn_greater_stealth )
			|| is_affected( wch, gsn_stealth ) || is_affected( wch, gsn_hide ) )
			&& !( IS_SET( ch->act, PLR_HOLYLIGHT ) ) )
			continue;
		...
		* Figure out what to print for class.
		*/
		strcpy( buf_class, class_table[wch->class].who_name );
		switch ( wch->level )
		{
		default: break;
		case MAX_LEVEL - 0:
			strcpy( buf_class, "{120}GOD{030}" );
			break;
		case MAX_LEVEL - 1:
		case MAX_LEVEL - 2:
		case MAX_LEVEL - 3:
			strcpy( buf_class, "{120}IMM{030}" );
			break;
		}
		if ( wch->pcdata->clan_army == 1 && wch->level < MAX_LEVEL - 3 )
		{
			plen=str_apd_max( pbuf, "{160}", plen, MAX_STRING_LENGTH);

			if ( *buf_class >='a' && *buf_class <= 'z' )
				*buf_class -= ('a' - 'A');

			if( *(buf_class+1) >='a' && *(buf_class+1) <= 'z' )
				*(buf_class+1) -= ('a' - 'A');

			if( *(buf_class+2) >='a' && *(buf_class+2) <= 'z' )
				*(buf_class+2) -= ('a' - 'A');
		}
		plen = str_apd_max( pbuf, buf_class, plen, MAX_STRING_LENGTH);
		plen = str_apd_max( pbuf, " ", plen, MAX_STRING_LENGTH);
		if ( wch->pcdata->clan_army == 1 )
			plen = str_apd_max( pbuf, "{030}", plen, MAX_STRING_LENGTH);

		strcpy(buf_race,race_table[wch->race].race_name);
		buf_race[3]='\0';
		if(wch->level>MAX_LEVEL-1)
			strcpy(buf_race,"---");
		...
		if(typ==1)
		{
			...
			else
			{
				...
				{
					sprintf(buf2, "%-13s {002}%-15s ",wch->name, wch->pcdata->clan_name);
					buf2[29]='\0';
				}
				else
				{
			...
			if( (wch->pcdata->switched || wch->desc !=NULL) &&
				!IS_AFFECTED( wch, AFF_STEALTH ) && !IS_AFFECTED( wch, AFF_HIDE ) )
			{
...
void do_search( CHAR_DATA *ch, char *argument )
{
	...
	if (arg[0] == '\0')
	{
		sprintf(buf, "%sSearch for what?", ANSI_GREEN_BOLD);
		send_to_char(buf, ch);
		found = TRUE;
	}
	else
	{
		...
		for(obj = ch->first_carrying; obj != NULL; obj = obj->next_content)
		{
			if(obj->wear_loc == WEAR_NONE && can_see_obj(ch, obj))
			{
				...
				{
					if(strstr(obj->short_descr, arg) != NULL)
					{
						if(found==FALSE)
						{
							sprintf(buf,"%sYou search through your things and find:", ANSI_GREEN_BOLD);
							send_to_char(buf, ch);
						}
						sprintf(buf,"%s%s %sin your inventory.",
								ANSI_WHITE_BOLD, format_obj_to_char(obj, ch, fShort), ANSI_YELLOW_DIM);
						send_to_char(buf,ch);
						found = TRUE;
					}

					sprintf(tbuf, "%s%s", ANSI_WHITE_BOLD, format_obj_to_char(obj, ch, fShort));
					for(obj2 = obj->first_content; obj2 != NULL; obj2 = obj2->next_content)
					{
						if(can_see_obj(ch, obj2) && strstr(obj2->short_descr, arg) != NULL)
						{
							if(found==FALSE)
							{
								sprintf(buf,"%sYou search through your things and find:", ANSI_GREEN_BOLD);
							...
							sprintf(buf,"%s%s %sin %s.", ANSI_WHITE_BOLD, 
									format_obj_to_char(obj2, ch, fShort), ANSI_YELLOW_DIM, tbuf);
							send_to_char(buf, ch);
							found = TRUE;
						}
					}
	...
	if(found == FALSE)
	{
		sprintf(buf,"%sYou aren't carrying '%s%s%s'.", ANSI_GREEN_BOLD, ANSI_YELLOW_DIM, arg, ANSI_GREEN_BOLD);
		send_to_char(buf, ch);
	}
	return;
...
void do_where( CHAR_DATA *ch, char *argument )
{
	...
	*sss = '\0';
	...
	if ( arg[0] == '\0' || doAll )
	{
		sprintf(buf, "%sPlayer           %sLocation                  %sLeader            %sAddress          \n\r",
				ANSI_YELLOW_BOLD, ANSI_MAGENTA_BOLD, ANSI_BLUE_BOLD, ANSI_CYAN_BOLD);
		...
		for( fpl = first_player; fpl!= NULL; fpl = fpl->next )
		{
			...
			if( victim->in_room != NULL)
				...
				{
					found = TRUE;
					sprintf(buf2, "%s%s", ANSI_YELLOW_DIM, victim->name);
					if(victim->in_room->area==ch->in_room->area)
						sprintf(buf3, "%s%s", ANSI_MAGENTA_DIM, victim->in_room->name);
					else
					{
						sprintf(buf3, "%s%s", ANSI_MAGENTA_DIM, victim->in_room->area->name);
					}
					if(victim->leader!=NULL && is_same_group( victim, victim->leader) &&
						victim != victim->leader )
						sprintf(buf4, "%s%s", ANSI_BLUE_DIM, victim->leader->name);
					else
					switch(test)
					{
						case 0: sprintf( buf4, "%s---", ANSI_BLUE_DIM); break;
						case 1: sprintf( buf4, "%s---", ANSI_BLUE_DIM); break;
						case 2: sprintf( buf4, "%s---", ANSI_BLUE_DIM); break;
						case 3: sprintf( buf4, "%s---", ANSI_BLUE_DIM); break;
						case 4: sprintf( buf4, "%s---", ANSI_BLUE_DIM); break;
						case 5: sprintf( buf4, "%s---", ANSI_BLUE_DIM); break;
					}
					test++;
					if(test>5)
						test=0;
					...
					if(victim->desc==NULL)
						sprintf(buf5, "%slocation.unknown", ANSI_CYAN_DIM);
					else
						if ( ch->level > 96 )
							sprintf( buf5, "%s%s", ANSI_CYAN_DIM, victim->desc->host );
						else
							sprintf( buf5, "%s%s", ANSI_CYAN_DIM, victim->desc->domain );

					sprintf( buf, " %s %s %s %s\n\r",
						str_resize(buf2,tbuf1,-25),
						str_resize(buf3,tbuf2,-35),
						str_resize(buf4,tbuf3,-27),
						str_resize(buf5,tbuf4,-27));
					leng = str_apd_max( sss, buf, leng, MAX_STRING_LENGTH );
				}
		}
		if ( !found )
	...
	*buf='\0';
	...
	if ( argument[0] == '\0')
	{
		...
		for(cnt=0;cnt<MAX_ALIAS;cnt++)
		{
			...
			{
				tot++;
				sprintf( tbuf, "{170}%-12s {030}%s\n\r", ch->pcdata->alias_c[cnt],ch->pcdata->alias[cnt] );
	...
	if (found!=4096)
	{
		...
		*/
		...
		for(ptt=argument; *ptt!='\0'; ptt++)
		{
			...
			if(percent > 3)
			{
				send_to_char( "Illegal alias usage!\n\r", ch );
				sprintf( tbuf, "%s used an illegal alias with IP: %s", ch->name, ch->desc->host );
				log_string( tbuf );
				log_string( argument );
				return;
			}
			...
			/*
			if we find an '&' we have a new command in the alias,
			start counting the '%' again, starting from zero.
			this way you will be able to :
			alias boo chat %&say %&talk %
			but you will hang with
			alias boo chat %&tell % %&talk %
			since tell has two %s.
			Manwe
			*/
			if(*ptt == '&')
			{
		...
		for ( sn = 0; sn < MAX_SKILL; sn++ )
		{
			...
			if(cnt!=-1)
			{
				...
				if(ch->mclass[ch->class] < skill_table[sn].skill_level[ch->class])
					tstc='*';
				sprintf(buf3,"%s", capitalize(skill_table[sn].name));
				buf3[18]='\0';
				sprintf( buf2, "%s%s%s%c%s%2d%s/%2d%%%s", ANSI_YELLOW_DIM, buf,
					str_resize(buf3,tbuf1,19), tstc, (ch->pcdata->learned[sn]>=adept) ? ANSI_YELLOW_BOLD : ANSI_YELLOW_DIM, ch->pcdata->learned[sn],
					ANSI_YELLOW_BOLD, adept, ANSI_YELLOW_DIM );
				strcpy(buf,buf2);
				if ( ++col % 3 == 0 )
				{
					strcat(buf,"\n\r");
...
void do_wimpy( CHAR_DATA *ch, char *argument )
{
	...
	if ( wimpy > ( ch->max_hit / 2 ) )
	{
		send_to_char( "Such cowardice ill becomes you. Try something lower.\n\r", ch );
		return;
	}
...
{
	char buf[MAX_STRING_LENGTH],buf2[MAX_STRING_LENGTH],tbuf[MAX_STRING_LENGTH],tbuf2[MAX_STRING_LENGTH];
	int sn;
	int col, cnt, lev, cls;
	char arg[3];

	/* Rewritten by Chaos 10/10/93 */
	/* Color added by Nodd 08/04   */
	...
	for(cnt=0;cnt<MAX_CLASS;cnt++)
	{
		...
		send_to_char( buf2, ch);
		for(lev=0;lev<MAX_LEVEL-3;lev++)
			for(sn=0;sn<MAX_SKILL;sn++)
				if(skill_table[sn].name != NULL)
					if(skill_table[sn].skill_level[cnt]==lev)
					{	
						sprintf( buf2, "%s%2d%s-%s%s", 
							ANSI_YELLOW_DIM, lev, ANSI_YELLOW_BOLD, ANSI_YELLOW_BOLD, 
							capitalize(skill_table[sn].name) );
						buf2[52] = ' ';
						sprintf(tbuf2, "%s", str_resize(buf2, tbuf, -53));
						strcat( buf, tbuf2);
...
void do_decline ( CHAR_DATA *ch, char *argument )
{
	...
	if( ch->pcdata->channel == 0 || ch->pcdata->channel == 1 )
	{
		send_to_char( "You cannot decline access to that channel.\n\r", ch );
		return;
	}
...
void do_config( CHAR_DATA *ch, char *argument )
{
	...
	if ( *arg1 == 'c' || *arg1=='C' )
	{
		...
		case 'j': case 'J':
			...
			break;
		case 'k': case 'K':
			bit2 = PLR2_NEWCHAT; /* Added by Emp */
			break;
		case 'l': case 'L':
			bit2 = PLR2_CHAOSSAY; /* Added by Emp */
			break;
		...
		strcat( buf, !IS_SET(ch->act, PLR_CHAT)
			? "{150} (E) {120}[ On  ] {010}[CHAT ] {030}You hear mud chatter.\n\r"
			: "{150} (E) {020}[ Off ] {010}[chat ] {030}You do not hear mud chatter.\n\r");
		...
		strcat( buf, !IS_SET(ch->pcdata->player2_bits, PLR2_NEWCHAT)
			? "{150} (K) {120}[ On  ] {010}[NEWCHAT ] {030}You hear newbie chatter.\n\r"
			: "{150} (K) {020}[ Off ] {010}[newchat ] {030}You do not hear newbie chatter.\n\r");

		/* Added new flag - Emp */

		strcat( buf, !IS_SET(ch->pcdata->player2_bits, PLR2_CHAOSSAY)
			? "{150} (L) {120}[ On  ] {010}[CHAOSSAY] {030}You speak Chaos-speak.\n\r"
			: "{150} (L) {020}[ Off ] {010}[chaossay] {030}You do not speak Chaos-speak.\n\r");
...
void do_disguise( CHAR_DATA *ch, char *argument )
{
	...
	char newString[MAX_STRING_LENGTH], code[MAX_STRING_LENGTH], 
		foreground[MAX_STRING_LENGTH], background[MAX_STRING_LENGTH], full[MAX_STRING_LENGTH];
	int index = 0;
	int leng = 0;
	...
	for(index = 0; index <= strlen(argument) && counter <= 80; index++)
	{
		if(argument[index] != '{')
		{
			sprintf(buf, "%c", argument[index]);
			leng = str_apd_max(newString, buf, leng, MAX_STRING_LENGTH);
			counter++;
		}
		else
		{
			if(argument[index+4] == '}')
			{
				...
				if (  isdigit(argument[index+1]) &&
						isdigit(argument[index+2]) &&
						isdigit(argument[index+3]))
				{
					...
					strcat (full, foreground);
	
					sprintf(code, "%s", full);
					leng = str_apd_max(newString, code, leng, MAX_STRING_LENGTH);
					index += 4;
				}
				else
				{ 
	...
	ch->long_descr = STRALLOC(newString);
	send_to_char( "Ok.\n\r", ch );
}
...
void do_spy( CHAR_DATA *ch, char *argument )
{
	...
	if((victim!=NULL)&&(!IS_NPC(victim))&&(victim->in_room!=ch->in_room))
		for(door=0;door<6;door++)
			...
			{
				get_spy_score( victim, ch );
				send_to_char( get_spy_score_txt, ch );
				return;
			}
...
void do_history( CHAR_DATA *ch, char *argument )
{
	...
	if( ch->pcdata->army_status > 0 )
	{
		if( ch->pcdata->army_status == 1 )
			sprintf( buf, "Race Army Status: Active Kills: %3d Losses: %3d",
			ch->pcdata->history[HISTORY_KILL_ARMY],
			ch->pcdata->history[HISTORY_LOSE_ARMY]);
		else
			sprintf( buf, "Race Army Status: Retired Kills: %3d Losses: %3d",
			ch->pcdata->history[HISTORY_KILL_ARMY],
			ch->pcdata->history[HISTORY_LOSE_ARMY]);
		act( buf, ch, NULL, NULL, TO_ROOM );
	...
	if( ch->pcdata->clan_army > 0 )
	{
		if( ch->pcdata->clan_army == 1 )
			sprintf( buf, "Clan Army Status: Active Kills: %3d Losses: %3d\n\r",
			ch->pcdata->history[HISTORY_KILL_CLAN],
			ch->pcdata->history[HISTORY_LOSE_CLAN]);
		else
			sprintf( buf, "Clan Army Status: Retired Kills: %3d Losses: %3d\n\r",
			ch->pcdata->history[HISTORY_KILL_CLAN],
			ch->pcdata->history[HISTORY_LOSE_CLAN]);
		act( buf, ch, NULL, NULL, TO_ROOM );
		act( buf, ch, NULL, NULL, TO_CHAR );
...
void do_usage( CHAR_DATA *ch, char *argument )
{
	...
	{
		if( ( IS_SET( fpl->ch->act, PLR_WIZINVIS) || is_affected( fpl->ch, gsn_greater_stealth ) )
...
void do_glance( CHAR_DATA *ch, char *argument )
{
	...
	if(( victim = get_char_room_even_hidden( ch, arg1 ) ) != NULL )
	{
		...
		if(IS_NPC(ch) || (number_percent() < ch->pcdata->learned[gsn_glance]))
		{
			...
			// Don't un-wizinvis imms when they glance -- Merzik 11/2/2004
...
void check_most( CHAR_DATA *ch )
{
	...
	if( IS_NPC(ch) || IS_IMMORTAL(ch) || ch->which_god==GOD_POLICE )
		return;

	/*
	* Changed most exp to show exp minus lost exp so it's not just
	* the first 95 nec to log on... Merzik 12-27-00
	*/

	if( ch->exp > ch->exp_lost )
		if( ( ch->exp - ch->exp_lost ) > Most_Values[ MOST_EXP ] )
		{
			Most_Values[ MOST_EXP ] = ( ch->exp - ch->exp_lost );
		...
		if( which_god(ch)==GOD_CHAOS || which_god(ch)==GOD_ORDER ||
			...
			if( which_god(ch)==GOD_CHAOS || which_god(ch)==GOD_ORDER ||
				...
				/* Getting rid of stupid berserking rangers here... Merzik - 10-23-00
				...
				*/
				...
				if ( ch->pcdata->eqdamroll > Most_Values[ MOST_DR ] )
				{
					Most_Values[ MOST_DR ] = ch->pcdata->eqdamroll;
					strcpy( Most_Names[ MOST_DR ], get_name( ch ) );
				}
				...
				*/
				...
				if ( ch->pcdata->eqhitroll > Most_Values[ MOST_HR ] )
				{
					Most_Values[ MOST_HR ] = ch->pcdata->eqhitroll;
					strcpy( Most_Names[ MOST_HR ], get_name( ch ) );
				}
...
void do_victory_list( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char_color( "{130}-------------------------------- VICTORY LIST --------------------------------\n\r", ch );
	send_to_char_color( "{130}             Victor                  Defeated                  Time{140}\n\r", ch );
	...
	else
	{
		...
		*pt='\0';
		...
		if( !strcasecmp( first_word, "join" ) )
		{
			...
			if( ch->level < 10 )
			{
				send_to_char( "You must be 10th level to join.\n\r",ch);
				return;
			}
			if ( !check_password(ch, argument) )
			{
...
void do_clanarmy( CHAR_DATA *ch, char *argument )
{
	char arg[MAX_INPUT_LENGTH];
	...
	if ( argument == NULL || *argument == '\0' )
		switch( ch->pcdata->clan_army )
	{
		default:
			return;

		case 0:
			send_to_char( "You have never joined your clan's army.\n\r", ch );
			return;

		case 1:
			send_to_char( "You are currently active in your clan's army.\n\r", ch );
			return;

		case 2:
			send_to_char( "You have retired from your clan's army.\n\r", ch );
			return;
	}

	argument = one_argument( argument, arg );

	if ( !strcasecmp( arg, "join" ) )
	{
		if ( which_god( ch ) == GOD_DEMISE || which_god( ch ) == GOD_POLICE )
		{
			send_to_char( "Your current status does not allow you to join an army.\n\r",ch);
			return;
		}
		...
		if ( ch->pcdata->clan_army == 1 )
		{
			send_to_char( "You are already in the clan army!\n\r",ch);
			return;
		}

		if ( ch->pcdata->clan_army > 1 )
		{
			send_to_char( "You may not rejoin the clan army!\n\r",ch);
			return;
		}

		if ( ch->in_room != room_index[ch->pcdata->clan->home] )
		{
			send_to_char( "You must be at your clan home room to join.\n\r",ch);
			return;
		}
		...
		send_to_char( "You join your clan army!\n\r", ch );
		ch->pcdata->clan_army = 1;
		return;
	}

	if ( !strcasecmp( arg, "retire" ) )
	{
		if ( ch->pcdata->clan_army < 1 )
		{
			send_to_char( "You are not in the clan army!\n\r", ch );
			return;
		}
		...
		if ( ch->in_room != room_index[ch->pcdata->clan->home] )
		{
			send_to_char( "You must be at your clan home room to retire.\n\r", ch );
			return;
		}
		...
		send_to_char( "You retire from your clan army.\n\r", ch );
		ch->pcdata->clan_army = 2;
		return;
	}
	return;
}

void do_tank( CHAR_DATA *ch, char *argument )
{
	char arg[MAX_INPUT_LENGTH];
	...
	if ( argument == NULL || *argument == '\0' )
	{
		if ( IS_SET( ch->pcdata->player2_bits, PLR2_TANK ) )
			send_to_char( "You are currently a teaching tank.\n\r", ch );
		else
			send_to_char( "You are not currently a teaching tank.\n\r", ch );
		return;
	}
	...
	if ( !strcasecmp( arg, "join" ) )
	{
		if ( which_god( ch ) == GOD_DEMISE || which_god( ch ) == GOD_POLICE )
		{
			send_to_char( "Your current status does not allow you to become a tank.\n\r",ch);
			return;
		}

		if ( ch->pcdata->tank_change != 1 )
		{
			send_to_char( "You can not change your tank status without gaining a level.\n\r", ch );
			return;
		}

		if ( IS_SET( ch->pcdata->player2_bits, PLR2_TANK ) )
		{
			send_to_char( "You are already a teaching tank!\n\r",ch);
			return;
		}
		...
		send_to_char( "You become a teaching tank!\n\r", ch );
		SET_BIT( ch->pcdata->player2_bits, PLR2_TANK );
		return;
	}
	...
	if ( !strcasecmp( arg, "retire" ) )
	{
		if ( !IS_SET( ch->pcdata->player2_bits, PLR2_TANK ) )
		{
			send_to_char( "You are not a teaching tank!\n\r", ch );
			return;
		}
		...
		send_to_char( "You retire from teaching.\n\r", ch );
		REMOVE_BIT( ch->pcdata->player2_bits, PLR2_TANK );
		ch->pcdata->tank_change = 0;
		return;
	}
	return;
}
