...
int apply_ac( OBJ_DATA *obj, int iWear )
{
	...
	case WEAR_BODY:
		...
		if ( obj->carried_by->pcdata->learned[gsn_armor_usage] > 0 )
			return(int)((((3.0*obj->carried_by->pcdata->learned[gsn_armor_usage])/100)+3)*obj->value[0]);
		else
			if ( obj->carried_by->race == RACE_DWARF ) /* Dwarves get a natural 100% armor usage skill */
				return 6 * obj->value[0];
			else
				return 3 * obj->value[0];
...
void extract_char( CHAR_DATA *ch, bool fPull )
{
	...
	for ( wch = first_char; wch != NULL; wch = wch->next )
	{
		if( wch->reply == ch )
			wch->reply = NULL;
		if( !IS_NPC( wch ) && wch->pcdata->replylock == ch )
			wch->pcdata->replylock = NULL;
	}
...
/*
* Find a char in the room (disregarding illusions)
* Added to fix quest bit
*/
CHAR_DATA *get_char_room_no_illusion( CHAR_DATA *ch, char *argument )
{
	char arg[MAX_INPUT_LENGTH];
	CHAR_DATA *rch;
	int number;
...
int char_exists(char *arg)
{
	...
	*/
	...
	strcpy(pfile, get_player_filename(arg, PFILE_TYPE_NORMAL));
...
char *get_name( CHAR_DATA *ch )
{
	...
	*buf='\0';
	...
	if( !IS_NPC( ch ) )
	{
		...
		{
			if( ch->sex == 2 )
				strcpy( get_name_buffer, "Enchantress " );
			else
				strcpy( get_name_buffer, "Wizard " );
			strcpy( buf, ch->name );
			*buf = UPPER( *buf );
		...
		if( ch->pcdata->pvnum == 107 && !strcasecmp(ch->name,"order"))
		{
			strcpy( get_name_buffer, "Lord " );
			strcpy( buf, ch->name );
			*buf = UPPER( *buf );
			strcat( get_name_buffer, buf);
			return( get_name_buffer );
		...
		if( ch->pcdata->pvnum == 166 && !strcasecmp(ch->name,"chaos"))
		{
			strcpy( get_name_buffer, "Lord " );
			strcpy( buf, ch->name );
			*buf = UPPER( *buf );
		...
		if( ch->pcdata->pvnum == 167 && !strcasecmp(ch->name,"demise"))
		{
			strcpy( get_name_buffer, "Lord " );
			strcpy( buf, ch->name );
			*buf = UPPER( *buf );
		...
		{
			if( ch->sex == 2 )
				strcpy( get_name_buffer, "Demigoddess " );
			else
				strcpy( get_name_buffer, "Demigod " );
			strcpy( buf, ch->name );
			*buf = UPPER( *buf );
			strcat( get_name_buffer, buf);
			return( get_name_buffer);
			...
			{
				if( ch->sex == 2 )
					strcpy( get_name_buffer, "High Enchantress " );
				else
				...
				*buf = UPPER( *buf );
				...
				if( ch->level > 96 )
				{
					...
					*buf = UPPER( *buf );
					...
					{
						if( ch->sex == 2 )
							strcpy( get_name_buffer, "Creator " );
						else
							strcpy( get_name_buffer, "Creator " );
					}
					...
					*buf = UPPER( *buf );
					...
					else if( ch->level > 90 )
					{
							...
							switch( ch->class )
						{
							...
							case CLASS_MONK:
								if( ch->sex == 2 )
									strcpy( get_name_buffer, "Kunoichi " );
								else
									strcpy( get_name_buffer, "Ninja " );
								break;
						}
						else
