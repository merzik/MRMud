					 ...
					 OBJ_DATA *obj, void *vo, CHAR_DATA *rndm)
{
	...
	*argpt = '\0';
	...
	else/* there should be an operator and value, so get them */
	{
		while ( ( *point != ' ' ) && ( !isalnum((int) *point ) ) )
			if ( *point == '\0' )
			{
				bug ( "Mob: %u ifchck operator without value", mob->pIndexData->vnum );
				return -1;
			}
			else
				*oprpt++ = *point++;
	...
	*/
	...
	if ( !strcasecmp( buf, "armystatus" ) )
	{
		...
		{
		case 'i': return ( 0);
		case 't': if ( actor && !IS_NPC( actor ))
					  return ( actor->pcdata->army_status );
				  else return 0;
		case 'n': if ( vict && !IS_NPC(vict))
	...
	/* Reincarnation check -- Added 1-11-2001 - Merzik */

	if ( !strcasecmp( buf, "reinc" ) )/* Checks number of reincarnations */
	{ /* Mobs always return -1 */
		switch ( arg[1] )
		{
		case 'i':
			return -1;
		case 'n':
			if ( actor )
				if ( !IS_NPC( actor ) )
				{
					lhsvl = actor->pcdata->reincarnation;
					rhsvl = atol( val );
					return mprog_veval( lhsvl, opr, rhsvl );
				}
				else
		...
		case 't':
			if ( vict )
				if ( !IS_NPC( vict ) )
				{
					lhsvl = vict->pcdata->reincarnation;
					rhsvl = atol( val );
					return mprog_veval( lhsvl, opr, rhsvl );
				}
				else
		...
		case 'r':
			if ( rndm )
				if ( !IS_NPC( rndm ) )
				{
					lhsvl = rndm->pcdata->reincarnation;
					rhsvl = atol( val );
					return mprog_veval( lhsvl, opr, rhsvl );
				}
				else
				...
				return -1;
		default:
			bug ( "Mob: %u bad argument to 'reinc'", mob->pIndexData->vnum );
			return -1;
		}
	}
