...
void move_char( CHAR_DATA *ch, int door)
{
	...
	/* A blocked room shouldn't let people in or out -- Merzik */
	if( IS_SET(in_room->room_flags, ROOM_BLOCK))
	{
		if( IS_AFFECTED(ch, AFF_CLEAR))
		{
			in_room->room_flags-= ROOM_BLOCK;
			send_to_char("You managed to clear a path out of the room.\n\r", ch);
		}
		else
		{
			send_to_char( "You appear to be blocked in!\n\r", ch);
			return;
		}
	}
	...
	/*
	Don't let mobs be moved or wander into clanhalls unless it's a pet following
	its master or a plain mob (descriptor will be null) -- Merzik 2002-04-17
	*/
		...
		&& ch->in_room->creator_pvnum == ch->pcdata->pvnum)
	{
		...
		/* We don't need this in the logs -- Merzik - 9-7-00
...
void do_hide( CHAR_DATA *ch, char *argument)
{
	...
	if(multi(ch, gsn_hide)==-1 && !IS_NPC(ch) && ch->race != RACE_TROLL)
		return;


	if( IS_NPC(ch) || number_percent() < ch->pcdata->learned[gsn_hide] || ch->race == RACE_TROLL)
	{
		if( IS_NPC(ch)||number_percent()<ch->pcdata->learned[gsn_greater_hide] || ch->race == RACE_TROLL)
...
void do_recall( CHAR_DATA *ch, char *argument)
{
	...
	*/
	...
	if( ( victim = who_fighting(ch)) != NULL)
	{
		...
		/*
		Can't recall exp away under your current level exp (anti-quaking code)
		-- Merzik 3-27-01
		*/

		if( ( ch->exp - lose) < exp_level( ch->class, ch->level - 1))
			lose = 0;
				...
				if(which_god(victim)!=which_god(ch) && !IS_NPC(ch) && !IS_NPC(victim))
					if( ch->pcdata->pkill_type == KILL_GOD && victim->pcdata->pkill_type == KILL_GOD)
					{
						...
						if(which_god(victim)==GOD_CHAOS)
							sprintf(buf, "%s has recalled from the erratic frenzy of %s!", buf2, get_name(victim));

						if(which_god(victim)==GOD_ORDER)
							sprintf(buf, "%s has recalled from the righteous fury of %s!", buf2, get_name(victim));

						if(which_god(victim)==GOD_DEMISE)
							sprintf(buf, "%s has recalled from the dark fury of %s!", buf2, get_name(victim));
...
void do_stealth( CHAR_DATA *ch, char *argument)
{
	...
	if( IS_NPC(ch) || number_percent() < ch->pcdata->learned[gsn_stealth])
	{
		...
		/* Disabling greater stealth -- Merzik 8/6/2003
		if(!IS_NPC(ch)
