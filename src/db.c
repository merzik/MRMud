...
***************************************************************************
* *
* Features/Changes Added -- Merzik *
* *
* 11/24/01 - Added a commenting feature to area files. Functions *
* load_area, load_mobiles, load_objects, and load_rooms all check for *
* '@' at the beginning of a line and ignore the entire line if found. *
* *
***************************************************************************/
...
void area_reset_message(AREA_DATA *pArea);
...
sh_int gsn_dodge;
sh_int gsn_evade;
sh_int gsn_counterstrike;
sh_int gsn_sweep;
sh_int gsn_bodythrow;
sh_int gsn_hide;
sh_int gsn_peek;
	...
	*/
	{
		...
		for ( ; ; )
		{
			...
			strcat(strArea, fread_word(fpList));
#else
...
void load_helps( FILE *fp )
{
	...
	/* Load army HQs - added as a temp fix by Emperor 10/26/99 */
...
void load_owners( void)
{
	...
	fclose( fpList );
	log_string("Loaded owners successfully." );
...
void area_reset_message(AREA_DATA *pArea)
{
	...
	*/
	if (pArea->nplayer > 0)
	{
		iRandom = number_range(1,3);
		for (gpl = first_player; gpl != NULL; gpl = gpl->next)
		{
			pch = gpl->ch;
			if(IS_AWAKE(pch) && pch->in_room != NULL && pch->in_room->area == pArea)
			{
				if (pArea->resetmsg != NULL && pArea->resetmsg[0] != '\0' )
				{
					sprintf(buf, "%s\n\r", pArea->resetmsg);
				}
				else
...
void do_resetarea( CHAR_DATA *ch, char *arg)
{
	...
	if( arg[0]!='a')
	{
		...
		send_to_char( buf, ch);
		/* Adding return here - not sure why we would reset all 
		   areas if that wasn't specified -- Merzik 12/17/2003 */
		return;
	}
	for ( pArea = first_area; pArea != NULL; pArea = pArea->next )
		reset_area( pArea );
				...
				if( !str_prefix( argument, buf ) )
				{
					...
					for( hash=0; hash < MAX_KEY_HASH; hash++)
						for( room=room_index_hash[hash]; room != NULL; room=room->next )
							...
							if( IS_IMMORTAL( ch ) )
							{
								sprintf( buf, "First area vnum: %d\n\r", pArea->low_r_vnum );
								send_to_char( buf, ch );
								sprintf( buf, "Area file name: %s\n\r", pArea->filename );
								send_to_char( buf, ch );
							}
#ifdef AREA_KILL
...
/* Returns string for a player's file name -- Merzik 8/25/2002 */

char *get_player_filename(char *szName, int iType)
{
	static char strPfile[MAX_STRING_LENGTH];

#ifdef CODER_EDITION

	switch(iType)
	{
	case PFILE_TYPE_NORMAL:
		sprintf(strPfile, "%s/%s", PLAYER_DIR, capitalize(szName));
		break;
	case PFILE_TYPE_DELETE:
		sprintf(strPfile, "%s/%s.del", PLAYER_DIR, capitalize(szName));
		break;
	case PFILE_TYPE_TEMP:
	case PFILE_TYPE_BACKUPTEMP:
		sprintf(strPfile, "%s/%s.tmp", PLAYER_DIR, capitalize(szName));
		break;
	case PFILE_TYPE_BACKUP:
		sprintf(strPfile, "%s/%s.bak", PLAYER_DIR, capitalize(szName));
		break;
	}
	...
	/* Get the first letter so we know which directory to use */
	strcpy(strDir, szName);
	strDir[1]='\0';
	if(strDir[0] >= 'A' && strDir[0] <= 'Z')
		strDir[0] += ('a' - 'A');
	...
	if(COMPRESS_FILES)
	{
		...
		{
		case PFILE_TYPE_NORMAL:
			sprintf(strPfile, "%s/%s/%s.gz", PLAYER_DIR, strDir, capitalize_name(szName));
			break;
		case PFILE_TYPE_DELETE:
			sprintf(strPfile, "%s/%s/delete.%s.gz", PLAYER_DIR, strDir, capitalize_name(szName));
			break;
		case PFILE_TYPE_TEMP:
			sprintf(strPfile, "%s/%s/temp.%s.gz", PLAYER_DIR, strDir, capitalize_name(szName));
			break;
		case PFILE_TYPE_BACKUP:
			sprintf(strPfile, "%s/%s/bak/%s.gz", PLAYER_DIR, strDir, capitalize_name(szName));
			break;
		case PFILE_TYPE_BACKUPTEMP:
			sprintf(strPfile, "%s/%s/bak/temp.%s.gz", PLAYER_DIR, strDir, capitalize_name(szName));
			break;
		}
	}
	else
	{
		...
		{
		case PFILE_TYPE_NORMAL:
			sprintf(strPfile, "%s/%s/%s", PLAYER_DIR, strDir, capitalize_name(szName));
			break;
		case PFILE_TYPE_DELETE:
			sprintf(strPfile, "%s/%s/delete.%s", PLAYER_DIR, strDir, capitalize_name(szName));
			break;
		case PFILE_TYPE_TEMP:
			sprintf(strPfile, "%s/%s/temp.%s", PLAYER_DIR, strDir, capitalize_name(szName));
			break;
		case PFILE_TYPE_BACKUP:
			sprintf(strPfile, "%s/%s/bak/%s", PLAYER_DIR, strDir, capitalize_name(szName));
			break;
		case PFILE_TYPE_BACKUPTEMP:
			sprintf(strPfile, "%s/%s/bak/temp.%s", PLAYER_DIR, strDir, capitalize_name(szName));
			break;
		}
	}
...
/* Added quiet logging feature to track admin usage -- Merzik 5-5-2001 */

void log_quiet( char *strLogEntry )
{
	char *strTime;
	FILE *fpQuietLog;
	char buf[MAX_INPUT_LENGTH];

	fpQuietLog = fopen( "quiet.log", "a");
...
void load_victors(void)
{
	...
	fpVictors = fopen( VICTORY_LIST, "r" );
	if ( NULL == fpVictors )
	{
		log_string( "Unable to open VICTORY_LIST." );
		return;
	}

	log_string( "Loading victory list..." );

	szStart = fread_string( fpVictors );

	if ( strcmp( szStart, "VICTORY_LIST" ) )
	{
		log_string( "Unexpected VICTORY_LIST format." );
		fclose( fpVictors );
		return;
	}

	for ( cnt = 0; cnt < VICTORY_LIST_SIZE; cnt++ )
		victory_list[cnt] = fread_string( fpVictors );

	/* victory_list[cnt].szVictoryText = fread_string( fpVictors ); */

	fclose( fpVictors );
	log_string( "Victory list loaded successfully." );
...
void save_victors( void )
{
	...
	if ( is_valid_save( VICTORY_LIST_TMP, "Victorlist" ) )
	{
		...
		log_string( "Victory list saved successfully." );
	}
...
/* Why we're saving usage in the siteban file I can't figure out...
Rewriting usage/siteban load and save routines -- Merzik 9-22-01
...
-- End of old siteban save/load */

/* New routines for loading siteban and usage data -- Merzik */
...
void load_sites( void )
{
	...
	fpSiteban = fopen( SITEBAN_FILE, "r" );

	if( fpSiteban == NULL )
	{
		log_printf( "Error opening %s for reading!", SITEBAN_FILE );
		return;
	}

	szBuffer = fread_string( fpSiteban );

	if( strcmp( szBuffer, "SITEBAN_LIST" ) )
	{
		log_string( "Unexpected SITEBAN_LIST format." );
		fclose( fpSiteban );
		return;
	}

	while( !feof( fpSiteban ) )
	{
		szBuffer = fread_string( fpSiteban );
		if( szBuffer[0] == '#' )
			break;
		CREATE( pban, BAN_DATA, 1 );
		pban->name = str_dup( szBuffer );
		pban->next = ban_list;
		ban_list = pban;
	}
	...
	log_string( "Siteban list loaded successfully." );
...
void save_sites( void )
{
	...
	BAN_DATA *pban, *pnextban;

	fpSiteban = fopen( SITEBAN_TEMP, "w");

	if( fpSiteban == NULL )
	{
		log_printf( "Error opening %s for writing!", SITEBAN_TEMP );
		return;
	}

	fprintf( fpSiteban, "SITEBAN_LIST~\n" );

	for( pban = ban_list; pban != NULL; pban = pnextban )
	{
		pnextban = pban->next;
		sprintf( strBuffer, "%s~\n", pban->name );
	...
	if ( is_valid_save( SITEBAN_TEMP, "Sitebanlist~" ) )
	{
		rename( SITEBAN_TEMP, SITEBAN_FILE );
		log_string( "Siteban list saved successfully." );
	}
...
void load_usage( void )
{
	...
	log_string( "Loading usage stats..." );

	fpUsage = fopen( USAGE_FILE, "r");

	if( fpUsage == NULL )
	{
		log_printf( "Error opening %s for reading!", USAGE_FILE );
		return;
	}

	szStart = fread_string( fpUsage );

	if ( strcmp( szStart, "USAGE_STATS" ) )
	{
		log_string( "Unexpected usage file format." );
		fclose( fpUsage );
		return;
	}
	...
	for( hour=0; hour<24; hour++ )
	{
		for( day=0; day < 7; day++ )
		{
			usage.players[hour][day] = fread_number( fpUsage );
		}
	}
	...
	for( hour=0; hour<24; hour++ )
	{
		for( day=0; day < 7; day++)
		{
			usage.recons[hour][day]=fread_number( fpUsage );
		}
	}

	fclose( fpUsage );
	log_string( "Usage stats loaded successfully." );
...
void save_usage( void )
{
	...
	fpUsage = fopen( USAGE_TEMP, "w");

	if( fpUsage == NULL )
	{
		log_printf( "Error opening %s for writing!", USAGE_TEMP );
		return;
	}

	fprintf( fpUsage, "USAGE_STATS~\n" );

	for(hour=0;hour<24;hour++)
	{
		fprintf(fpUsage, "%ld %ld %ld %ld %ld %ld %ld\n", usage.players[hour][0],
			usage.players[hour][1], usage.players[hour][2],
			usage.players[hour][3], usage.players[hour][4],
			usage.players[hour][5], usage.players[hour][6]);
	}
	...
	fprintf( fpUsage, "#Usagestats\n" );

	/* Let's make sure this works. - Chaos 4/25/99 */
	fclose( fpUsage );
	if( is_valid_save( USAGE_TEMP, "Usagestats" ) )
	{
		rename( USAGE_TEMP, USAGE_FILE );
		log_string( "Usage stats saved successfully" );
	}
...
/* Initializes first pvnum, etc. from config.txt file -- 08-05-2002 - Merzik */
...
void load_config(void)
{
	...
	log_string("Loading config file...");

	if((fpConfig = fopen(CONFIG_FILE, "r")) == NULL)
	{
		log_printf("Error opening %s for reading!", CONFIG_FILE);
		return;
	}

	szBuffer = fread_word(fpConfig);
	if(strcasecmp(szBuffer, "Pvnum"))
	{
		log_printf("Encountered unexpected value while reading %s: \"%s\"", CONFIG_FILE, szBuffer);
		abort();
	}

	next_pvnum = fread_number(fpConfig);
	log_printf("First available pvnum: %ld", next_pvnum);

	fclose(fpConfig);
	log_string("Config file loaded successfully.");
	return;
}
...
void save_config(void)
{
	...
	if((fpConfig = fopen( CONFIG_TEMP, "w")) == NULL)
	{
		log_printf("Error opening %s for writing!", CONFIG_TEMP);
		return;
	}

	fprintf(fpConfig, "Pvnum %ld\n", next_pvnum);
	fprintf(fpConfig, "#Config\n");

	fclose(fpConfig);

	if( is_valid_save(CONFIG_TEMP, "Config"))
	{
		rename(CONFIG_TEMP, CONFIG_FILE);
		log_string("Config file saved successfully.");
	}
	else
	{
		log_printf("An error was encountered while saving %s!", CONFIG_TEMP);
	}
...
void load_bounties(void)
{
	...
	if((fp=fopen(BOUNTY_FILE, "r")))
	{
		...
		while(done==0);
		fclose(fp);
		log_string("Bounty list loaded successfully.");
	}
	return;
}
...
int get_obj_stat( OBJ_INDEX_DATA *obj_index, char *argument )
{
	int iStat = 0;
	AFFECT_DATA *pAffect;
	...
	if ( !strcmp( argument, "armor" ) )
	{
		if ( obj_index->item_type == ITEM_ARMOR )
			iStat -= obj_index->value[0];

		for( pAffect = obj_index->first_affect; pAffect; pAffect = pAffect->next )
			if ( pAffect->location == APPLY_AC )
				iStat += pAffect->modifier;

		return( iStat );
	}

	if ( !strcmp( argument, "damroll" ) )
	{
		for( pAffect = obj_index->first_affect; pAffect; pAffect = pAffect->next )
			if ( pAffect->location == APPLY_DAMROLL )
				iStat += pAffect->modifier;

		return( iStat );
	}

	if ( !strcmp( argument, "hitroll" ) )
	{
		for( pAffect = obj_index->first_affect; pAffect; pAffect = pAffect->next )
			if ( pAffect->location == APPLY_HITROLL )
				iStat += pAffect->modifier;

		return( iStat );
	}

	if ( !strcmp( argument, "savevsspell" ) )
	{
		for( pAffect = obj_index->first_affect; pAffect; pAffect = pAffect->next )
			if ( pAffect->location == APPLY_SAVING_SPELL )
				iStat += pAffect->modifier;

		return( iStat );
	}

	if ( !strcmp( argument, "hp" ) )
	{
		for( pAffect = obj_index->first_affect; pAffect; pAffect = pAffect->next )
			if ( pAffect->location == APPLY_HIT )
				iStat += pAffect->modifier;
	...
	{
		for( pAffect = obj_index->first_affect; pAffect; pAffect = pAffect->next )
			if ( pAffect->location == APPLY_MANA )
				iStat += pAffect->modifier;
	...
	{
		for( pAffect = obj_index->first_affect; pAffect; pAffect = pAffect->next )
			if ( pAffect->location == APPLY_MOVE )
				iStat += pAffect->modifier;
