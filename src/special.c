...
bool gahld_kingghost(CHAR_DATA *ch)
{
	...
	/* Converting this picky C code to mob progs in gahld.are
	-- Merzik - 10-8-00

	if((tobj = get_obj_list(ch,"cherish", ch->first_carrying)))
	{
	STRFREE (tobj->name);
	tobj->name=STRALLOC("coin unique qpwoeirutyalsk");
	...
	if((tobj = get_obj_list(ch,"qpwoeirutyalsk", ch->first_carrying)))
	{
	AFFECT_DATA *paf;
...
bool gahld_spectre(CHAR_DATA *ch)
{
	...
	if(number_percent()>98)
		for(tch=ch->in_room->first_person;tch!=NULL;tch=tch->next_in_room)
			...
			{
				act(
					"The Ancient King says 'I have taken to haunting this castle because my own blood employed the help of a deceitful magician. The magician became a Lich and drained all of the life from my great grandson and his subjects. I wish to be avenged, but all I have been able to do is haunt this room. Something will not let me venture forth.'"
					,ch,NULL,NULL,TO_ROOM);
				return 0;
			}
			return 0;
...
bool gahld_lich(CHAR_DATA *ch)
{
	...
	for(tch=ch->in_room->first_person;tch!=NULL;tch=tch->next_in_room)
		...
		case 11:
			act(
				"The Lich says 'Once upon a time, there was a great kingdom with many subjects. One day, I decided to obliterate the pesky mortals. I did.'",
				ch,NULL,NULL,TO_ROOM);
			break;
		case 12:
