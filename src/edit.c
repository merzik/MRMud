...
char * const a_flags [] =
{
	...
	*/
	...
	void do_mcreate( CHAR_DATA *ch, char *argument )
	{
		...
		/* this is for OLC... disabled for now
		...
		*/
		...
		if (!fullcont)
		{
			...
			if (ch->pcdata->castle==NULL)
			{
				...
				send_to_char("You need to have a castle before you can use MCREATE.\n\r", ch);
				return;
			}
			...
			if (ch->in_room->creator_pvnum!=ch->pcdata->pvnum)
			{
				...
				send_to_char("You must be in your castle to use MCREATE.\n\r", ch);
				return;
			}
			...
			if(ch->gold<mob_cost)
			{
				ch_printf(ch ,"Sorry! You don't have enough gold to do an MCREATE.\n\r");
				ch_printf(ch ,"It will cost you %d coins.\n\r",mob_cost);
				return;
			}
			...
			if(ch->pcdata->castle->num_mobiles >= ch->level/3+2)
			{
				...
				ch_printf(ch,"For your level, you are allowed %d mobiles in your castle.\n\r",ch->level/3+2);
				return;
			}
			...
			else
			{
				...
				if(vnum>=27000)
				{
					send_to_char("Sorry! All the mobile vnums are gone.\n\r",ch);
					send_to_char("Please inform an administrator.\n\r",ch);
					return;
				}
	...
	/* ocreate not yet an option for castles...
	...
	*/
	...
	void do_mset( CHAR_DATA *ch, char *argument )
	{
		...
		case SUB_MOB_DESC:
			if (!ch->dest_buf)
			{
				send_to_char("Fatal error: report to an administrator.\n\r", ch);
		...
		*/
		...
		/* temporary definition of fullcont */
		...
		if ( arg1[0] == '\0' || arg2[0] == '\0' )
		{
			if (fullcont)
			{
				...
				send_to_char( " str int wis dex con sex class level race\n\r", ch );
				send_to_char( " gold account hp mana move practice align exp\n\r", ch );
				send_to_char( " thirst drunk full language racearmy clanarmy\n\r", ch );
		...
		if (!fullcont)
		{
			...
			{
				ch_printf(ch ,"Sorry! You don't have enough gold to do an MSET.\n\r");
				ch_printf(ch ,"It will cost you %d coins.\n\r",mset_cost);
				return;
			}
		}
		...
		if( ch->level < 99 )
		{
			...
			else if ( !strcasecmp( arg2, "level" ) )
			{
				...
				{
					ch_printf(ch, "Sorry! The level range is 0 to %d.\n\r", ch->level/2 );
					return;
				}
			...
			else if ( !strcasecmp( arg2, "race" ) )
			{
				...
				{
					send_to_char( "You must specify a race as well:\n\r", ch);
			...
			if (fullcont)
			{
				...
				*/
				...
				if ( !strcasecmp( arg2, "clanarmy" ) )
				{
					...
					if ( value < 0 || value > 2 )
					{
						send_to_char( "Clan army status is between 0 and 2.\n\r", ch );
						return;
					}

					victim->pcdata->clan_army = value;
					return;
				}

				if ( !strcasecmp( arg2, "racearmy" ) )
				{
					if ( IS_NPC( victim ) )
					{
						send_to_char( "Not on NPC's.\n\r", ch );
					...
					if ( value < 0 || value > 2 )
					{
						send_to_char( "Race army status is between 0 and 2.\n\r", ch );
						return;
					}

					victim->pcdata->army_status = value;
					return;
				}
				...
				if ( !strcasecmp( arg2, "account" ) )
				{
					victim->pcdata->account = value;
					return;
				}
	...
	void save_rooms( FILE *fp, AREA_DATA *area)
	{
		...
		for ( vnum = area->low_r_vnum; vnum <= area->hi_r_vnum; vnum++ )
		{
			...
			if(room->creator_pvnum!=0)
				fprintf( fp, "%ld ", room->creator_pvnum);
			else
	...
	void do_goto( CHAR_DATA *ch, char *argument )
	{
		...
		sprintf( strLog, "%-12s Using: Goto Arg: %s", ch->name, argument );
		log_quiet( strLog );
