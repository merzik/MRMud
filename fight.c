...
bool check_sweep args((CHAR_DATA *ch, CHAR_DATA *victim));
bool check_martial_arts(CHAR_DATA *ch, CHAR_DATA *victim);
...
int exp_compute args((CHAR_DATA *ch, CHAR_DATA *victim));
...
int count_chars_fighting_char args((CHAR_DATA *victim));
...
/*
	Count number of players that are fighting a particular player
*/
int count_chars_fighting_char(CHAR_DATA *pVictim)
{
	int iCount = 0;
	PLAYER_GAME *pChar;
	PLAYER_GAME *pNextChar;
	
	for (pChar = first_player; pChar != NULL; pChar = pNextChar)
	{
		pNextChar = pChar->next;
		
		if (who_fighting(pChar->ch) == pVictim)
		{
			iCount++;
		}
	}
				...
				if(pd->constant_duration <= 0)
				{
					if(ppd == NULL)
					{
						if(IS_NPC(ch))
							...
							ch->pcdata->poison = npd; /* seems to cause problems...*/
					}
					else
						ppd->next = npd;
					DISPOSE(pd);
...
void multi_hit(CHAR_DATA *ch, CHAR_DATA *victim, int dt)
{
	...
	if(IS_NPC(victim))
		...
		if(!IS_NPC(ch) && ch->race==RACE_OGRE)
			...
			/*
			Attempt martial arts hits - doesn't allow normal hits plus martial arts
			-- Merzik 2-26-02
			*/

			if(check_martial_arts(ch, victim))
				return;
...
bool check_hit(CHAR_DATA *ch, CHAR_DATA *victim, int hit_roll, int dam_type)
{
	...
	*/
	...
	else if(dam_type == gsn_bodythrow)
	{
		cnt = multi(ch,gsn_bodythrow);
		if(cnt == -1)
			return(FALSE);
		level = ch->mclass[ cnt ] + (ch->level - ch->mclass[cnt]) / 3;
		scale = 15;
	}
	else if(dam_type == gsn_counterstrike)
	{
		cnt = multi(ch,gsn_counterstrike);
		if(cnt == -1)
			return(FALSE);
		level = ch->mclass[ cnt ] + (ch->level - ch->mclass[cnt]) / 3;
		scale = 15;
...
void one_hit(CHAR_DATA *ch, CHAR_DATA *victim, int dt)
{
	...
	*/
	...
	/* This makes a looooong wait in pkill...
	...
	*/
	...
	else
	{
		if((wield != NULL) && (wield->item_type == ITEM_WEAPON))
		{
			...
			/* Bonus for Dwarf using Axe -- Merzik 7-21-01 */
			if(ch->race == RACE_DWARF && wield->value[3] == 12)
				dam = dam * 5 / 4;
		}
		else
			/* Doubled monk damage -- Merzik 9-11-00 */
			dam=number_range(1,4+ch->level/6)+ch->mclass[CLASS_MONK]*4/3;
	}
	...
	*/
	...
	if((dt == gsn_knife || dt == gsn_twist) && (multi(ch, gsn_knife)!=-1 || IS_NPC(ch)))
	{
		if(!IS_NPC(ch))
		{
			if(dt == gsn_twist)
				dam = dam*(30 + ch->mclass[multi(ch,gsn_twist)])/20;
			else
				dam = dam*(30 + ch->mclass[multi(ch,gsn_knife)])/30;
	...
	if(dt == gsn_quick_draw && (multi(ch, gsn_knife)!=-1 || IS_NPC(ch)))
	{
		if(!IS_NPC(ch))
		{
			dam = dam*(30 + ch->mclass[multi(ch,gsn_knife)])/45; /* Order 11/16/93 */
			/* now check to see if it's a critical knife */
			if(!IS_AFFECTED(victim,AFF2_BLEEDING))
				if(number_percent() < 25 && multi(ch,gsn_critical_hit) != -1 &&
					number_percent() < ch->pcdata->learned[gsn_critical_hit])
			...
			dam = dam*(30 + ch->level/4)/45;/* Order 11/16/93 */
	}
			...
			if(victim->position == POS_DEAD)
			{
				...
				if(victim->in_room->area->low_r_vnum != ROOM_VNUM_ARENA && !IS_NPC(victim))
				{
					...
					if(!IS_NPC(ch))
					{
						...
						if(ch->pcdata->pkill_type == victim->pcdata->pkill_type)
							switch(ch->pcdata->pkill_type)
						{
							case KILL_NONE:
							case KILL_MOB:
								break;
							case KILL_GOD:
								{
									...
									save_char_obj(ch, NORMAL_SAVE); /* save killer */
									break;
								}
							case KILL_RACE:
								{
							...
							case KILL_CLAN:
								{
									/* Add to player's death and kill totals */
									victim->pcdata->history[HISTORY_LOSE_CLAN]++;
									ch->pcdata->history[HISTORY_KILL_CLAN]++;

									/* Update clan death and kill totals */
									ch->pcdata->clan->pkills[CLAN_ARMY_KILL]++;
									victim->pcdata->clan->pdeaths[CLAN_ARMY_DEATH]++;

									/* Save updated clan and player files */
									check_killer(ch, victim);
									save_clan(ch->pcdata->clan);
									save_clan(victim->pcdata->clan);
									save_char_obj(ch, NORMAL_SAVE);
						...
						{
							bug("Pkill type mismatch on death - %s killing %s.", ch->name, victim->name);

							/* Checking for god war victory first, since it'll be the primary pkill venue */
						...
						if(victim != ch)
						{
							...
							sprintf(buf, "Z%12s (%dx Level %2d) on %s", victim->name,
								victim->pcdata->reincarnation, victim->level,
				...
				if(!IS_NPC(victim) )
				{
					sprintf(log_buf, "%s killed by %s at %u", victim->name, get_name(ch),
						victim->in_room->vnum);
					log_string(log_buf);
					...
					if(victim->level < 2)
						...
						if(victim->level == 95)
							...
							if(victim->exp>exp_level(victim->class,(victim->level)-1)+1)
								gain_exp(victim, exp_level(victim->class,
								(victim->level)-1)+1 - victim->exp);

					/* Knight's Death
					-- Temporarily disabled for revamping -- Merzik 10-31-00
			...
			*/
			{
				...
				if((dt == gsn_backstab && dam > 1600 && number_percent()<ch->pcdata->learned[gsn_greater_backstab])
					|| (dt == gsn_twist && number_percent() < ch->pcdata->learned[gsn_twist]))
			...
			if(victim != ch)
			{
				...
				*/
				if(dt >= TYPE_HIT || dt == gsn_martial_arts)
				{
...
bool check_parry(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	/* log_printf("Parry check: %s hitting %s", ch->name, victim->name); */
...
bool check_dodge(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	if(IS_NPC(victim))
		iChance = UMIN(25, victim->level/3+5);
	else
		if(victim->race == RACE_HALFLING)
			iChance = (25 + victim->level / 10);
		else
		{
			if(multi(victim, gsn_dodge) == -1)
				return(FALSE);
			iChance = victim->pcdata->learned[gsn_dodge] / 3;
		}

		if(OGRE_INTIMIDATE)
			iChance -= 10;

		// Adjust for differences in levels
		iChance += (victim->level - ch->level) / 4;

		// Add or subtract for dexterity level
		if(!IS_NPC(victim))
		{
			iChance += victim->pcdata->perm_dex - (10 + (victim->level / 10));
			iChance += (get_curr_dex(victim) - victim->pcdata->perm_dex) / 2;
		}

		if(number_percent() >= iChance)
			return FALSE;
...
/*
* Check for evade and counterstrike.
*/
bool check_evade(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	if(multi(victim, gsn_evade) == -1)
		return(FALSE);

	iChance = victim->pcdata->learned[gsn_evade] / 2;
	...
	if(IS_SET(victim->attack, 8))
		return FALSE;
	else
		victim->attack+=8;

	if(multi(victim, gsn_counterstrike) != -1)
		bStrike = TRUE;

	if(check_hit(victim, ch, 0-GET_HITROLL(victim), gsn_counterstrike)
		&& number_percent() < victim->pcdata->learned[gsn_counterstrike] && bStrike)
	{
		int iDamage = 0;
		if(IS_NPC(victim) || !IS_SET(victim->pcdata->spam, 8))
			act("You evade and counter $n's attack.", ch, NULL, victim, TO_VICT);
		if(IS_NPC(ch) || !IS_SET(ch->pcdata->spam, 2))
			act("$N evades and counters your attack.", ch, NULL, victim, TO_CHAR);
		iDamage = number_range(GET_DAMROLL(victim), GET_DAMROLL(victim) * 3);
		damage(victim, ch, iDamage, gsn_counterstrike);
	}
	else
	{
		if(IS_NPC(victim) || !IS_SET(victim->pcdata->spam, 8))
...
/*
* Check for monk leg sweep.
*
* ch = Player fighting victim... Victim = Player trying to flee
*/
bool check_sweep(CHAR_DATA *ch, CHAR_DATA *victim)
{
	int iChance;
	...
	if(multi(ch, gsn_sweep) == -1)
		return(FALSE);

	iChance = (ch->pcdata->perm_dex + ch->pcdata->perm_int) * ch->pcdata->learned[gsn_sweep] / 100;

	if (iChance < 1)
		return(FALSE);

	// log_printf("%d %s vs %s", iChance, ch->name, victim->name);

	if(number_percent() >= iChance + (ch->level - victim->level) / 4)
		return FALSE;
	...
	act("$n's quick leg sweep foils your flee attempt!", ch, NULL, victim, TO_VICT);
	act("Your quick leg sweep foils $N's flee attempt!", ch, NULL, victim, TO_CHAR);
...
void raw_kill(CHAR_DATA *victim)
{
	...
	for(fch=first_char; fch != NULL; fch=fch->next)
	{
		if (who_fighting(fch) == victim && IS_AFFECTED(fch, AFF_CHARM) &&
			IS_NPC(fch) && IS_SET(fch->act, ACT_ONE_FIGHT))
			SET_BIT(fch->act, ACT_WILL_DIE);
...
void group_gain(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	*/
	int iMembers[6];
	char strBuffer[MAX_INPUT_LENGTH];
	CHAR_DATA *gch;
	OBJ_DATA *pObj, *pTmpObj, *pObj_Next;

	for(iCnt = 0; iCnt < 6; iCnt++)
		iMembers[iCnt] = 0;

	/* Dummy checks - Killing self or pets */
	...
	/* Check the group makeup and collect stats */

	for(gch = first_char; gch != NULL; gch = gch->next)
	{
		if(is_same_group(ch, gch))
		{
			if(IS_IMMORTAL(gch)) /* No exp when grouped with immortal */
				return;
			...
			if(IS_NPC(gch))
			{
				...
				if(IS_SET(gch->pcdata->player2_bits, PLR2_TANK))
				{
					iTanks++;
					if(gch->level < iMembers[3] || iMembers[3] == 0)
						iMembers[3] = gch->level;
					if(gch->level > iMembers[4] || iMembers[4] == 0)
						iMembers[4] = gch->level;
				}
				else
				{
					iLevelers++;
					iTotalLevels += gch->level;
					if(gch->level < iMembers[0] || iMembers[0] == 0)
						iMembers[0] = gch->level;
					if(gch->level > iMembers[1] || iMembers[1] == 0)
						iMembers[1] = gch->level;
				}
		}
	}
	...
	if(iLevelers < 1)
		...
		iMembers[2] = iTotalLevels / iLevelers;
	/*
	for(iCnt = 0; iCnt < 6; iCnt++)
	log_printf("Members array %d: %d", iCnt, iMembers[iCnt]);
	*/
	if(iTanks + iLevelers < 1)
	{
		sprintf(strBuffer, "Group_gain: Player: %s - Tanks plus Levelers = %d", ch->name, iTanks + iLevelers);
		bug(strBuffer);
		return;
	}
	else
	{
		if(iTotalLevels / (iTanks + iLevelers) > 95)
		{
			sprintf(strBuffer, "Group_gain: Group size: %d Total levels: %d", iTanks + iLevelers, iTotalLevels);
			bug(strBuffer);
			return;
		}
	}

	/* Figure exp and pay out */

	iGroupExp = xp_compute(ch, victim);
	/*
	sprintf(strBuffer, "Tanks: %d Levelers: %d Group Exp: %d Total Levels: %d",
	iTanks, iLevelers, iGroupExp, iTotalLevels);
	log_string(strBuffer);
	*/
	switch(iTanks)
	{
	case 0:
	...
	if(iGroupExp < iLevelers)
		iGroupExp = iLevelers; /* Everyone will get 1 exp minimum */

	if(iLevelers < 1)
		iExpShare = 0;
	else
		iExpShare = iGroupExp / iLevelers;
	/*
	sprintf(strBuffer, "Penalty: %d Adj Total Exp: %d Exp Share: %d", iPenalty, iGroupExp, iExpShare);
	log_string(strBuffer);
	*/
	for(gch = first_char; gch != NULL; gch = gch->next)
	{
		if(!is_same_group(ch, gch) || gch->in_room != ch->in_room || IS_AFFECTED(gch, AFF_CHARM))
			continue;

		if(IS_SET(gch->pcdata->player2_bits, PLR2_TANK))
		{
			send_to_combat_char("You feel rewarded in teaching your followers.\n\r", gch);
		}
		else
		{
			iExp = iExpShare * gch->level / (iTotalLevels / iLevelers);

			iExp = number_range(iExp * 8 / 10, iExp * 12 / 10);

			for(iCnt = 0; iCnt < gch->pcdata->reincarnation; iCnt++)
				iExp = iExp * 5/9;
			...
			iExp -= (iExp * iPenalty / 100);

			if(gch->position == POS_SLEEPING)
				iExp = 0;

			sprintf(strBuffer, "You receive %d experience points.\n\r", iExp);
			send_to_combat_char(strBuffer, gch);
			gain_exp(gch, iExp);
			/*
			iExp = exp_compute(gch, victim);
			iExp -= (iExp * iPenalty / 100);
			sprintf(strBuffer, "You would receive %d experience points with new code.\n\r", iExp);
			send_to_combat_char(strBuffer, gch);
			*/
		}

		if(!IS_NPC(gch))
			for(pObj = gch->first_carrying; pObj != NULL; pObj = pObj_Next)
			{
				pObj_Next = pObj->next_content;
				if(pObj->wear_loc == WEAR_NONE)
					continue;

				if((IS_OBJ_STAT(pObj, ITEM_ANTI_EVIL) && IS_EVIL(gch) )
					|| (IS_OBJ_STAT(pObj, ITEM_ANTI_GOOD) && IS_GOOD(gch) )
					|| (IS_OBJ_STAT(pObj, ITEM_ANTI_NEUTRAL) && IS_NEUTRAL(gch)))
				{
					act("You can no longer wear $p until your alignment is fixed.", gch, pObj, NULL, TO_CHAR);
					act("$n is zapped by $p.", gch, pObj, NULL, TO_ROOM);

					if(pObj == get_eq_char(gch, WEAR_WIELD)
						&& (pTmpObj = get_eq_char(gch, WEAR_DUAL_WIELD)) != NULL)
						pTmpObj->wear_loc = WEAR_WIELD;
					unequip_char(gch, pObj);
				}
			}
	}
}

/* Revised code to figure out experience for killing something
-- Merzik 9-20-2001 */

int exp_compute(CHAR_DATA *ch, CHAR_DATA *victim)
{
	int iExp, iCnt, iLevelDiff;

	if(!IS_NPC(victim))
		return(0);

	/* Compute flat experience amount */

	iExp = victim->level * victim->level;
	iExp += victim->max_hit * 30;
	if(victim->npcdata->damnodice != 0 && victim->npcdata->damsizedice != 0)
		iExp += victim->npcdata->damnodice * victim->npcdata->damnodice * victim->npcdata->damsizedice * victim->npcdata->damsizedice;
	if(victim->npcdata->damplus != 0)
		iExp += victim->npcdata->damplus * victim->npcdata->damplus;
	iExp += victim->npcdata->armor * victim->npcdata->armor / 3;

	/* Inflate flat amount based on char/victim level and make reinc/level adjustments */

	iExp = (iExp * 5); // Don't ask where the number came from...

	iLevelDiff = abs(victim->level - (ch->level * 6 / 5));

	if(iLevelDiff > 15)
		iExp = iExp / (iLevelDiff / 15);

	for(iCnt = 0; iCnt < ch->pcdata->reincarnation; iCnt++)
		iExp = iExp * 1/2;

	if(ch->level == 90)
		iExp /= 2;

	if(ch->level > 90 && ch->level <= 95)
		iExp = 3 * iExp / 4;
...
{
	"hit", "slice", "stab", "slash", "whip",
		"claw", "blast", "pound", "crush", "grep",
		"bite", "pierce", "chop", "hit",
		"combination punch", "thrust kick", "palm punch",
		"spinning back-hand", "jump kick", "round house",
		"spinning hook kick", "double roundhouse", "flying scissor kick",
		"knee", "claw", "bite", "head butt",
		"elbow slam", "uppercut", "hooves"
};
			...
			else
			{
				if(dt == gsn_martial_arts)
				{
					if(martial_arts_attack > 0)
						attack = attack_table[13+martial_arts_attack];
					else
						attack = attack_table[22-martial_arts_attack];
				}
				else
					if(dt == TYPE_HOOVES)
						attack = attack_table[29];
...
void do_trip(CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_combat_char("You can't trip someone who is flying!\n\r", ch);
		return;
	}
...
void do_bodythrow(CHAR_DATA *ch, char *argument)
{
	CHAR_DATA *victim;
	int cnt, iChance;
	...
	if(IS_NPC(ch))
		...
		iChance = ((ch->pcdata->learned[gsn_bodythrow] / 2) + (ch->level - victim->level));

	if(iChance > 95)
		iChance = 95; /* Nobody's perfect */

	if(iChance < 5)
		iChance = 5; /* Let's give them a small chance at least */

	if(number_percent() < iChance)
	{
		act("$n throws you to the ground!", ch, NULL, victim, TO_VICT);
		act("You throw $N down to the ground!", ch, NULL, victim, TO_CHAR);
		act("$n throws $N to the ground!", ch, NULL, victim, TO_NOTVICT);
		if(!IS_NPC(ch))
			if(number_percent() < (ch->mclass[cnt] + ch->pcdata->learned[gsn_bodythrow]) / 2)
			{
				int iDam = 0;
				iDam = number_range(GET_DAMROLL(ch), GET_DAMROLL(ch) * 2);
				damage(ch, victim, iDam, gsn_bodythrow);
			}
	...
	{
		act("$n lunges at you violently!", ch, NULL, victim, TO_VICT);
		act("You lunge at $N, but fail to get ahold of $M!", ch, NULL, victim, TO_CHAR);
		act("$n lunges at $N, but misses!", ch, NULL, victim, TO_NOTVICT);
	}
	WAIT_STATE(ch, skill_table[gsn_bodythrow].beats*200/(200+ch->mclass[cnt]));
	return;
}
	...
	if(!found)
		...
		else
		{
			if(IS_AFFECTED(victim, AFF_CHARM) && victim->master != NULL)
			{
				...
				else /* Added check for a pet belonging to a player you're fighting so that rogues
					 have the ability to use their attack skill to kill enemy's pets -- Merzik */

					 if(IS_NPC(victim) && !IS_NPC(victim->master)
						 && victim->master == who_fighting(ch)
						 && multi(ch, gsn_attack) != -1)
					 {
						 int iChance = 0;
						 char strBuffer[MAX_INPUT_LENGTH];

						 if(IS_NPC(ch))
							 iChance = ch->level - victim->master->level;
						 else
							 iChance = ch->mclass[CLASS_ROGUE] + (ch->level - victim->master->level);

						 /* Make sure we're realistic... */
						 if(iChance < 1)
							 iChance = 1;
						 ...
						 if(number_percent() < iChance)
						 {
							 sprintf(strBuffer, "You violently slice the throat of %s's pet!\n\r",
								 get_name(victim->master));
							 send_to_combat_char(strBuffer, ch);

							 sprintf(strBuffer, "%s violently slices your pet's throat!\n\r", get_name(ch));
							 send_to_combat_char(strBuffer, victim->master);
						 ...
						 {
							 sprintf(strBuffer, "Your swipe at %s's pet misses horribly!\n\r",
								 get_name(victim->master));
							 send_to_combat_char(strBuffer, ch);
							 sprintf(strBuffer, "%s's swipe at your pet misses horribly!\n\r", get_name(ch));
							 send_to_combat_char(strBuffer, victim->master);
						 }
						 WAIT_STATE(ch, PULSE_VIOLENCE);
						 return;
					 }
					 send_to_combat_char("You cannot kill someone's slave.\n\r", ch);
					 return;
			}
		}
			...
			number_percent () < (float)ch->pcdata->learned[gsn_quick_draw]*.95)
		{
				...
				&&(obj->value[3]==11||obj->value[3]==2))
			{
				if(number_percent() < ch->pcdata->learned[gsn_knife])
					one_hit(ch, victim, gsn_quick_draw);
				else
					damage(ch, victim, 0, gsn_quick_draw);
			}
		}
		multi_hit(ch, victim, TYPE_UNDEFINED);
		return;
...
void add_to_victory_list(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	sprintf(buf, "%11s (%dx Level %2d) %11s (%dx Level %2d) %s",
		ch->name,
		ch->pcdata->reincarnation,
		ch->level,
		victim->name,
		victim->pcdata->reincarnation,
		victim->level,
		(char *) ctime(&current_time));
	buf[strlen(buf)-1] = '\0';

	/* Move everything up one... */
	for(cnt = 0; cnt < VICTORY_LIST_SIZE-1; cnt++)
	{
		memset(victory_list[cnt], 0, strlen(victory_list[cnt]));
		strcpy(victory_list[cnt], victory_list[cnt+1]);
	}

	/* Add new victory string to the end of the array */
	memset(victory_list[VICTORY_LIST_SIZE-1], 0, strlen(victory_list[VICTORY_LIST_SIZE-1]));
	strcpy(victory_list[VICTORY_LIST_SIZE-1], buf);
...
/* Handles starting fight for gang banging assassins because multi_hit triggers
a knife whenever an additional char hits an assassin -- Merzik 8/10/2003 */
void start_gangbang(CHAR_DATA *ch, CHAR_DATA *victim)
{
	if(ch == NULL || victim == NULL)
	{
		return;
	...
	if(!IS_NPC(ch) && ch->pcdata->auto_flags != AUTO_OFF && skill_lookup(ch->pcdata->auto_command) != -1)
	{
		int sn;
		sn = skill_lookup(ch->pcdata->auto_command);
		if(skill_table[sn].target == TAR_CHAR_OFFENSIVE && is_spell(sn))
		{
			char argbuf[MAX_INPUT_LENGTH];
			sprintf(argbuf, "'%s' %s", ch->pcdata->auto_command, victim->name);
			do_cast(ch, argbuf);
		}
		else
		{
...
void do_murder(CHAR_DATA *ch, char *argument)
{
	...
	if(IS_NPC(victim))
	{
		send_to_combat_char("You cannot murder creatures.\n\r", ch);
		return;
	}
	...
	if(!IS_NPC(victim))
		...
		/* Allow gang bang only on Demise pure asns 94 and lower */
		if(victim->fighting != NULL)
		{
			bool bStartFight = FALSE;

			if((victim->class == CLASS_ASSASSIN)
				&& (victim->mclass[CLASS_ASSASSIN] == victim->level)
				&& (victim->level < 95)
				&& (which_god(victim) == GOD_DEMISE)
				&& (ch->level <= victim->level + 10)
				&& (IS_SET(victim->act, PLR_KILLER)))
			{
				switch(victim->pcdata->reincarnation)
				{
					case 0:		/* 0x only gets one hunter */
						bStartFight = (count_chars_fighting_char(victim) == 0);
						break;
					case 1:		/* 1x can only have 2 hunters */
						bStartFight = (count_chars_fighting_char(victim) <= 1);
						break;
					default:	/* 2x and higher get unlimited gang bang */
						bStartFight = TRUE;
						break;
				}
			}
			...
			if (bStartFight)
			{
				start_gangbang(ch, victim);
				return;
			}
			else
			{
				send_to_combat_char("That person is currently fighting someone else.\n\r", ch);
				return;
			}
		}
		...
		/* Lagging players is 1) bad game design,
		2) not necessary,
		3) unpopular - Ken 12/26/99
		...
		*/
		...
		kill_type = get_attack_type(ch, victim);

		if(kill_type < KILL_MOB)
		{
		...
		*/
		...
		No attack limits on players with kflag -- Merzik 12-22-00 */

		if(!IS_NPC(ch) && !IS_NPC(victim) && kill_type != KILL_GOD)
			if(!check_add_attack(ch, victim) && !IS_SET(victim->act, PLR_KILLER))
			{
				char buf12[180];
			...
			if(!IS_NPC(victim))
			{
				if(kill_type == KILL_GOD)
				{
					char bufgod[MAX_INPUT_LENGTH];
					sprintf(bufgod, "%s", get_name(ch));
					sprintf(buf, "%s battles 'Help! I am being attacked by %s!'", get_name(victim), bufgod);
					do_battlespam(buf);
				}
				if(kill_type != KILL_ASN)
				{
					char bufvictim[MAX_INPUT_LENGTH];

					sprintf(bufvictim, "Help! I am being attacked by %s!", get_name(ch));
					do_shout(victim, bufvictim);
					victim->wait = 0; /* remove time penalty of shout */
				}
			}

			ch->pcdata->pkill_type = kill_type;
			victim->pcdata->pkill_type = kill_type;
			...
			if(IS_AFFECTED(victim, AFF2_HAS_FLASH) && !IS_NPC(ch) && !IS_NPC(victim))
			{
				if(number_percent() < victim->pcdata->learned[gsn_flash_powder]/2)
				{
					...
					{
						/* Using Blind is a big hassle... Let's just lag attacker instead so the
						assassin has some running time if he makes it out of the room -- Merzik
			...
			/* Mirror lag? Bleh. -- Merzik - 10-16-00
			*
			* if(!is_affected(ch, gsn_truesight) && has_mirror(ch, victim))
			* {
			* WAIT_STATE(ch, PULSE_VIOLENCE*2);
			...
			*/
			...
			{
				OBJ_DATA * obj;
				DID_QDRAW = TRUE; /* To prevent quickdraw AND spellcast in first round */
				if((obj=get_eq_char(ch,WEAR_WIELD))!=NULL
					&&(obj->value[3]==11||obj->value[3]==2))
				{
					if(number_percent() < ch->pcdata->learned[gsn_knife])
			...
			if(!IS_NPC(ch) && ch->pcdata->auto_flags!=AUTO_OFF &&
				...
				victim->hit >= victim->max_hit * 0.7 &&
				ch->mclass[CLASS_ROGUE]==ch->level)
				backstab(ch,victim);
			else

				/* Allow spellcasters to start a murder with a spell -- Merzik 12-9-00 */

				if(!IS_NPC(ch) && ch->pcdata->auto_flags != AUTO_OFF
					&& skill_lookup(ch->pcdata->auto_command) != -1 && !DID_QDRAW)
				{
					int sn;
					sn = skill_lookup(ch->pcdata->auto_command);
					if(skill_table[sn].target == TAR_CHAR_OFFENSIVE && is_spell(sn))
...
void do_backstab(CHAR_DATA *ch, char *argument)
{
	...
	if(!IS_NPC(victim) && ((IS_NPC(ch) && IS_AFFECTED(ch, AFF_CHARM)) ||
		...
		if((IS_NPC(ch) && IS_AFFECTED(ch, AFF_CHARM)) || !IS_NPC(ch))
			...
			if((victim->hit < victim->max_hit * 0.7)/*&&(victim->distracted<5)*/)
			{
				act("$N is too hurt and suspicious ... you can't sneak up.",
			...
			!(ch->race == RACE_AVIARAN && IS_OUTSIDE(ch))))
		{
			for(attempt = 0; attempt < 6; attempt++)
			{
				...
				if(!IS_NPC(ch))
				{
					...
					/* Anti-quaking measure - don't lose exp below current level exp
					-- Merzik 3-27-01
					*/
					if((ch->exp - exp) < exp_level(ch->class, ch->level - 1))
						exp = 0;
								...
								if(which_god(ch) == GOD_CHAOS || which_god(ch) == GOD_ORDER ||
									...
									if(which_god(victim) == GOD_CHAOS || which_god(victim) == GOD_ORDER ||
										...
										if(which_god(victim) != which_god(ch))
											if(ch->pcdata->pkill_type == KILL_GOD && victim->pcdata->pkill_type == KILL_GOD)
											{
												char buf2 [MAX_INPUT_LENGTH];
												sprintf(buf2, "%s", get_name(ch));
												if(ch->desc == NULL)
													sprintf(buf, "%s's arcane connection to the realm has been severed while in battle with %s!",
...
void do_kick(CHAR_DATA *ch, char *argument)
{
	...
	if(cnt==-1 && !IS_NPC(ch))
	{
		send_to_combat_char("You better learn how to kick first.\n\r", ch);
		return;
	}
...
void do_bash(CHAR_DATA *ch, char *argument)
{
	...
	if(cnt==-1 && !IS_NPC(ch))
	{
		send_to_combat_char("You better learn how to bash first.\n\r", ch);
		return;
	}
	...
	chance = ch->level/3;
	chance += (get_curr_dex(ch) + get_curr_str(ch) + get_curr_con(ch));
	chance -= (get_curr_dex(victim) + get_curr_int(victim) + get_curr_con(victim));
	chance += (ch->pcdata->perm_dex / 2);
	chance += (ch->pcdata->perm_str / 2);
	...
	{
		damage(ch, victim, number_range(GET_DAMROLL(ch), GET_DAMROLL(ch) * 4), gsn_bash);
...
/*
* Deals out martial arts hits for monks. Returns TRUE if a hit was
* attempted, FALSE if no hit was attempted (so a regular hit can be
* attempted). -- Merzik - 2002-02-20
*/

bool check_martial_arts(CHAR_DATA *ch, CHAR_DATA *victim)
{
	int iLevel=0, iPoints=0, iDamHigh=0, iDamLow=0, iNumHits=1, iMaxHits=0, iDamage=0;
	int iHighAttack=6, iMoveLoss=0;

	if(IS_NPC(ch) || get_eq_char(ch, WEAR_WIELD) || get_eq_char(ch, WEAR_DUAL_WIELD))
		return(FALSE);
	...
	iLevel = ch->mclass[CLASS_MONK];

	/* Martial arts must be at least level 16 -- Merzik */
	if (iLevel < 16)
		return(FALSE);

	/* Potential for having the bigger hits in the available pool */
	if (iLevel > 80 && (5 + (ch->max_move / 400) > number_percent()))
	{
		iHighAttack = 9;
	}
	else
	{
		if (iLevel > 60 && (5 + (ch->max_move / 300) > number_percent()))
		{
			iHighAttack = 8;
		}
		else
		{
			if (iLevel > 40 && (5 + (ch->max_move / 200) > number_percent()))
			{
				iHighAttack = 7;
			}
		}
	...
	iPoints = (iLevel + ch->pcdata->perm_dex + ch->pcdata->perm_int) * ch->pcdata->learned[gsn_martial_arts] / 1000;

	/* Small potential bonus for reincs */
	iPoints += (ch->pcdata->reincarnation * number_percent() / 100);
	// log_printf("Martial arts: %s with %d points and high attack of %d.", ch->name, iPoints, iHighAttack);
	...
	/* Gives a range between 1 and 5 */
	iMaxHits = 1 + (iLevel / 20);

	/* Give them one more possible hit if their movement is full enough */
	if (((ch->move / ch->max_move) * 33) > number_percent())
		iMaxHits++;

	/* Randomize the number of possible hits each round */
	if(iLevel < 50)
	{
		iMaxHits = number_range(1, iMaxHits);
	}
	else
	{
		iMaxHits = number_range(2, iMaxHits);
	}

	/* Added check to make sure both chars are in same room so hits don't work
	after recall, flee, or death -- Merzik 2002-05-01 */
	while(iNumHits <= iMaxHits && iPoints > 0 && ch->in_room == victim->in_room)

	{
		//log_printf("Martial arts: %s: %d points %d hits out of %d max", ch->name, iPoints, iNumHits, iMaxHits);
		iDamLow = (iLevel + ch->pcdata->perm_str + get_curr_str(ch)) / 3;

		switch(number_range(1, (iPoints > iHighAttack ? iHighAttack : iPoints)))
		{
		case 1:
			iMoveLoss = 2;
			iPoints -= 2;
			martial_arts_attack = 1;
			iDamHigh = iDamLow + GET_DAMROLL(ch);
			break;
		case 2:
			iMoveLoss = 2;
			iPoints -= 2;
			martial_arts_attack = 2;
			iDamHigh = iDamLow + GET_DAMROLL(ch) * 1.2;
			break;
		case 3:
			iMoveLoss = 3;
			iPoints -= 3;
			martial_arts_attack = 3;
			iDamHigh = iDamLow + GET_DAMROLL(ch) * 1.4;
			break;
		case 4:
			iMoveLoss = 3;
			iPoints -= 3;
			martial_arts_attack = 4;
			iDamHigh = iDamLow + GET_DAMROLL(ch) * 1.6;
			break;
		case 5:
			iMoveLoss = 4;
			iPoints -= 4;
			martial_arts_attack = 5;
			iDamHigh = iDamLow + GET_DAMROLL(ch) * 1.8;
			break;
		case 6:
			iMoveLoss = 4;
			iPoints -= 4;
			martial_arts_attack = 6;
			iDamHigh = iDamLow + GET_DAMROLL(ch) * 2;
			break;
		case 7:
			iMoveLoss = 5;
			iPoints -= 5;
			martial_arts_attack = 7;
			iDamHigh = iDamLow + GET_DAMROLL(ch) * 2.3;
			break;
		case 8:
			iMoveLoss = 5;
			iPoints -= 5;
			martial_arts_attack = 8;
			iDamHigh = iDamLow + GET_DAMROLL(ch) * 2.6;
			break;
		case 9:
			iMoveLoss = 6;
			iPoints -= 6;
			martial_arts_attack = 9;
			iDamHigh = iDamLow + GET_DAMROLL(ch) * 3;
			break;
		}

		iNumHits++;

		/* Make sure they have enough movement to even try */
		if (iMoveLoss > ch->move)
		{
			send_to_combat_char("You attempt to strike your foe, but can't muster the strength.\n\r", ch);
			break;
		}
		else
		{
		...
		if(iDamLow > iDamHigh)
			iDamHigh = number_range(iDamLow, iDamLow * 1.2);

		iDamage = GET_DAMROLL(ch);

		// Adjust damage to be proportional to % of pure monk levels
		iDamage += (number_range(iDamLow, iDamHigh) * ch->mclass[CLASS_MONK] / ch->level);

		//log_printf("Martial arts: Attack: %d Low: %d High: %d", martial_arts_attack, iDamLow, iDamHigh);

		if(number_percent() < ch->pcdata->learned[gsn_martial_arts] &&
			check_hit(ch, victim, 0-GET_HITROLL(ch), gsn_martial_arts))
			damage(ch, victim, iDamage, gsn_martial_arts);
		else
...
void do_slay(CHAR_DATA *ch, char *argument)
{
	...
	*/
	...
	if(!IS_NPC(ch))
		log_printf("%s slayed %s at %u", ch->name, victim->name, victim->in_room->vnum);
...
void do_slaughter(CHAR_DATA *ch, char *argument)
{
	...
	if(!IS_NPC(ch) )
	{
		log_printf("%s slaughtered %s at %u", ch->name, victim->name, victim->in_room->vnum);
...
void do_knife(CHAR_DATA *ch, char *argument)
{
	...
	if(IS_NPC(victim))
		...
		/* NEW KNIFE LAG ADDED TO WIMP ASSASSINS A BIT BY SCOTT/CHAOS 09/23/1999 ORIGINAL CODE BELOW COMMENT OUT */

		/* WAIT_STATE(ch, skill_table[gsn_knife].beats*100/(100+ch->mclass[cnt])); */
		WAIT_STATE(ch, skill_table[gsn_knife].beats*200/(200+ch->mclass[cnt]));
		...
		if(IS_NPC(ch))
			...
			if(number_percent() < ch->pcdata->learned[gsn_knife])
			{
				if((number_percent() < (ch->pcdata->perm_str + ch->pcdata->perm_dex) * 2)
					&& multi(ch, gsn_twist) != -1)
					one_hit(ch, victim, gsn_twist);
				else
					one_hit(ch, victim, gsn_knife);
			}
			else
...
/*	int exp_cost,level;*/
	...
	if(ch->in_room->area->low_r_vnum == ROOM_VNUM_ARENA)
	{
		send_to_combat_char("Why would you assassinate in the arena?\n\r", ch);
		return;
	}

	/* Make sure they have assassinate skill and they are a pure assassin */
	if(multi(ch, gsn_assassinate)==-1 || ch->class!=CLASS_ASSASSIN || ch->mclass[CLASS_ASSASSIN] != ch->level)
	{
		send_to_combat_char("Only pure assassins can assassinate.\n\r", ch);
		return;
	}

	/* Only demise followers can assassinate */
	if(which_god(ch) != GOD_DEMISE)
	{
		send_to_combat_char("You can not find it in your heart to assassinate that person.\n\r",ch);
		return;
	}
	/*
	level = ch->mclass[multi(ch,gsn_assassinate)];
	*/
	argument=one_argument(argument, arg);

	/* Cost to assassinate is dependent upon victim's level - Emperor 12/3/99 */
	/* Nobody liked this idea, Switching to 1/7 -- Merzik - 8-25-00
	And now switching to 1/10 again... Merzik 5-14-2001 */

	/*
	if(!IS_NPC(victim) && victim->level < 35)
	exp_cost=(exp_level(ch->class,level)-exp_level(ch->class,level-1))/3;
	else if(!IS_NPC(victim) && victim->level < 50)
	exp_cost=(exp_level(ch->class,level)-exp_level(ch->class,level-1))/4;
	else if(!IS_NPC(victim) && victim->level < 70)
	exp_cost=(exp_level(ch->class,level)-exp_level(ch->class,level-1))/6;
	else
	*/
	/*
	...
	*/
	if(arg[0] == '\0')
	{
		/* ch_printf(ch, "No target specified! Assassination experience cost is %d.\n\r", exp_cost); */
		send_to_combat_char("You must specify a target!", ch);
		return;
	}
	...
	/* Allow only assassination of chaos/order followers for level 95 chars */
	if(ch->level == 95 && !(which_god(victim) == GOD_ORDER || which_god(victim) == GOD_CHAOS))
	{
		send_to_combat_char("An assassin of YOUR caliber!? Go kill your true enemies!\n\r", ch);
		return;
	}
	...
	if(IS_AFFECTED(victim, AFF_CHARM) && victim->master == ch)
	{
		send_to_combat_char("You could just 'kill' it.\n\r", ch);
		return;
	}
	/* Removing experience cost for assassinate -- Merzik 8-6-2003
	...
	*/
	...
	if(!IS_NPC(victim))
		...
		/* Reinc based level restrictions for assassins */
/*
		switch(ch->pcdata->reincarnation)
		{
		case 0:
			if(victim->level < ch->level - 12)
			{
				send_to_combat_char("Victims must be no more than twelve levels below your level.\n\r", ch);
				return;
			}
			break;
		case 1:
			...
			{
				send_to_combat_char("Victims must be no more than eight levels below your level.\n\r", ch);
				return;
			}
			break;
		case 2:
			...
			{
				send_to_combat_char("Victims must be no more than four levels below your level.\n\r", ch);
				return;
			}
			break;
		case 3:
		...
		default:
			...
			{
				send_to_combat_char("Victims must be at least your level.\n\r", ch);
				return;
			}
			break;
		}
*/
		...
		{
			send_to_combat_char("Victims must be no more than ten levels below your level.\n\r", ch);
			return;
		}
		...
		if(!IS_NPC(victim))
			...
			{
				send_to_combat_char("The hand of their God weakens you.\n\r", ch);
				return;
			}

			if(!IS_NPC(victim) && which_god(victim) == GOD_POLICE)
			{
				send_to_combat_char("You may not assassinate that person you jackass!.\n\r",ch);
				return;
			}
			...
			if(!IS_NPC(victim))
			{
				...
				check_killer(ch, victim);
				ch->pcdata->pkill_type = KILL_ASN;
				victim->pcdata->pkill_type = KILL_ASN;

			}

			/* Why the hell would assassinate fail? Not like the asn would try to attack
			then stand there like a dumbass. -- Merzik 2002-04-17
			...
			*/
			...
			/* Removing chance of assassinate failure
			}
			else
			ch->asn_obj=NULL;
			*/
...
void mix_race_war(void)
{
	...
	for(tn=0; tn<MAX_RACE-1 ; tn++)
		Race_Battle_List[tn]=-1;

	for(found=FALSE, tn=0; tn<MAX_RACE-1; tn++, found=FALSE)
		while(!found)
		{
...
Modified can_attack to attack_type to return the type of battle being initiated -- Merzik */

sh_int get_attack_type(CHAR_DATA *ch, CHAR_DATA *victim)
{
	bool reincok=FALSE;
	...
	if(which_god(victim)==GOD_INIT_ORDER || which_god(victim)==GOD_INIT_CHAOS)
		return KILL_NONE;
	/* Demise followers are now assassins/GOD_POLICE no longer used -- Merzik 8/11/2003
	...
	*/
	...
	/* Demise should only assassinate, not murder */
	if(which_god(ch) == GOD_DEMISE)
		return KILL_NONE;

	/* Anyone can attack demise at 10 levels below or higher - recorded as assassin kill */
	if(which_god(victim) == GOD_DEMISE)
	{
...
bool check_add_attack(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	if(!IS_NPC(ch) && !IS_NPC(victim))
	{
		...
		if(god_war && total_pvnum_attacks >= 50)
			return(FALSE);
		if(!god_war && total_pvnum_attacks >= 48)
			return(FALSE);
...
void do_battle(CHAR_DATA *ch, char *argument)
{
	...
	if(arg[0] == '\0')
	{
		send_to_combat_char("Battle whom?\n\r", ch);
		return;
	}
	...
	if(IS_NPC(victim))
	{
		send_to_combat_char("You cannot battle with creatures.\n\r", ch);
		return;
	}

	if(victim->pcdata->clan == NULL)
	{
		send_to_char("You can't battle with someone not in a clan!\n\r", ch);
		return;
	}
	...
	{
		ch_printf(ch, "%s tells you 'No fighting in here! You thug!'\n\r",
			capitalize(find_keeper(ch)->short_descr));
		ch->reply = find_keeper(ch);
		return;
	}
	...
	if(ch->position == POS_FIGHTING)
	{
		send_to_combat_char("You're already fighting!\n\r", ch);
		return;
	}

	if(!can_battle(ch, victim))
	{
		send_to_char("You can not battle that player.\n\r", ch);
		return;
	}
	...
	if(victim->fighting != NULL)
	{
		send_to_combat_char("You may not battle that player at the moment.\n\r", ch);
		return;
	}
	...
	{
		char buf[180];
		sprintf(buf, "%s has been killed quite recently.\n\r", get_name(victim));
		send_to_combat_char(buf, ch);
		return;
	}
	...
	if(IS_AFFECTED(victim, AFF2_HAS_FLASH))
	{
		if(number_percent() < victim->pcdata->learned[gsn_flash_powder] / 2)
		{
			...
			REMOVE_BIT(victim->affected2_by, 0-AFF2_HAS_FLASH);
			act("$n throws some powder onto the ground and there is a bright flash!", victim,
				NULL, NULL, TO_ROOM);
			act("You throw some powder onto the ground and there is a bright flash!", victim,
				NULL, ch, TO_CHAR);
	...
	ch->pcdata->pkill_type = KILL_CLAN;
	victim->pcdata->pkill_type = KILL_CLAN;

	sprintf(buf, "I'm being attacked by %s!", get_name(ch));
	do_clantalk(victim, buf);
	do_shout(victim, buf);
	victim->wait = 0;

	/* Let's start this fight up -- Check for quickdraw, then backstab,
	then spell to initiate fight. Then start flailing. */

	/* Check for assassin quickdraw */
	if(number_percent () < (float)ch->pcdata->learned[gsn_quick_draw]*.75)
	{
		OBJ_DATA * obj;
		...
		if(ch->pcdata->auto_flags != AUTO_OFF
			...
			/* Check for offensive spell on auto */
			if(ch->pcdata->auto_flags != AUTO_OFF
				&& skill_lookup(ch->pcdata->auto_command) != -1)
			{
				int sn;
				sn = skill_lookup(ch->pcdata->auto_command);
				if(skill_table[sn].target == TAR_CHAR_OFFENSIVE && is_spell(sn))
...
void do_battl(CHAR_DATA *ch, char *argument)
{
	send_to_combat_char("If you want to BATTLE, spell it out!\n\r", ch);
	return;
}

bool can_battle(CHAR_DATA *ch, CHAR_DATA *victim)
{
	bool reinc_ok = FALSE;
	bool level_ok = FALSE;
	...
	if(ch->pcdata->clan == NULL || victim->pcdata->clan == NULL
		|| ch->pcdata->clan_army != 1 || victim->pcdata->clan_army != 1
		|| ch->pcdata->clan == victim->pcdata->clan)
		return FALSE;

	/* Check reinc restrictions */
	...
	case 3:
		{
			if(victim->pcdata->reincarnation > 1)
				reinc_ok = TRUE;
			break;
		}
	case 2:
		{
			if(victim->pcdata->reincarnation > 0)
				reinc_ok = TRUE;
			break;
		}
	case 1:
		{
			if(victim->pcdata->reincarnation < 3)
				reinc_ok = TRUE;
			break;
		}
	case 0:
		{
			if(victim->pcdata->reincarnation < 2)
				reinc_ok = TRUE;
			break;
		}
	default:
	...
	{
		if(victim->level >= (ch->level * 7/8) - 2)
			level_ok = TRUE;
	}
	else
	{
