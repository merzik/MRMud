...
bool chat_enabled; /* Allow or disallow global chat */
...
int main( int argc, char **argv )
{
	...
	*/
	...
	/* Move this after time is initialised; add checks for open file lims
	* - Drevlin 07/09/2002 */
	{
		char bufm[200];
		struct rlimit rlpt;
		...
		snprintf( bufm, sizeof(bufm), "System memory usage: %ld max %ld current",
		...
		getrlimit( RLIMIT_DATA, &rlpt );
		snprintf(bufm, sizeof(bufm), "Unlimited memory usage: %ld max %ld current",
			(long)rlpt.rlim_max, (long)rlpt.rlim_cur);
		log_string( bufm );

		getrlimit(RLIMIT_OFILE, &rlpt);
		snprintf(bufm, sizeof(bufm), "System open file limits: %ld max %ld cur",
			(long)rlpt.rlim_max, (long)rlpt.rlim_cur);
		log_string(bufm);
		rlpt.rlim_max = RLIM_INFINITY;
		rlpt.rlim_cur = RLIM_INFINITY;
		setrlimit(RLIMIT_OFILE, &rlpt);
		getrlimit(RLIMIT_OFILE, &rlpt);
		snprintf(bufm, sizeof(bufm), "New open file limits: %ld max %ld cur",
			(long)rlpt.rlim_max, (long)rlpt.rlim_cur);
		log_string(bufm);
	}
	...
	*/
	...
	/* Enabling true game behavior for tmuds... Merzik - 10-22-00
...
void close_socket( DESCRIPTOR_DATA *dclose , bool Force)
{
	...
	if ( Force && ( ch = dclose->character ) != NULL )
	{
		...
		/*
		* Kill off switch with mob (Homonc/possess/etc.)
		* to prevent abuse and crashes -- Merzik 12-11-00
		*/
...
bool nanny( DESCRIPTOR_DATA *d, char *argument )
{
		...
		switch ( d->connected )
	{
		...
		case CON_GET_NAME:
			...
			*/
			if ( ( IS_SET(ch->act, PLR_DENY) && !TEST_GAME) || ( ch->pcdata->last_time < 946700000 ) )
			{
				if( ch->level >= 99 )
				...
				log_string( log_buf );
				write_to_buffer( d, "You are denied access either because of a corrupted character or\n\rbecause your character is too old. Please contact an admin to\n\rresolve the situation.\n\r", 0 );
				close_socket( d , TRUE);
				return(outp);
			}
			...
			if( !strcasecmp( argument, "new" ) )
			{
				...
				write_to_buffer( d, "\n\rChoosing a name is one of the most important parts of this game...\n\r"
					...
					"Suitable names are an essential part of maintaining a role-playing\n\r"
					"environment. Stupid names, names which involve profanity, or sexual\n\r"
					"implications, or names that use illegal words will NOT be tolerated.\n\r"
					"Names that include two words or more are also not allowed.\n\r\n\r"
					"Stupid Names: Dorkone, Dorktwo, Tdork, BGut, FFFunnyyy\n\r"
					"Illegal Words: Lord, Lady, Sir, Miss, Maiden, Squire, Thee\n\r"
					"Multiple Words: TheBigOne, LordHolio, SirNerd, FrogBoy\n\r"
					"Similar Names: Rhapsody and Rhapsodys\n\r\n\r"
					"After reading above, please choose a name for your character: ", 1000000);
		...
		case CON_GET_NEW_NAME:
			/* Added check for . and .. (directory names crash the game) -- 6/9/2003 Merzik */
			if(!strcasecmp(argument, "new") || (*argument == '.'))
			{
		...
		case CON_READ_MOTD:
			...
			if ( ch->level == 0 )
			{
				...
				*/
				...
				log_printf( "New character '%s' created on %s", ch->name, ch->desc->host );
			}
			...
			if( TEST_GAME)
			{
				send_to_char( "You are entering a temporary version of MrMud.\n\r", ch);
				send_to_char( "We will take no responsibility for things wrong here.\n\r", ch);
				send_to_char( "We will take no responsibility for problems to your character.\n\r", ch);
			}
...
bool check_parse_name( char *name , bool mobcheck)
{
	...
	*/
	if ( is_name( name, "new chaos emperor order" ) )
		return TRUE;
	if ( is_name( name, "the lord king queen cop police duke duchess count countess sir maiden all auto immortal self someone god supreme demigod ass fuck shit piss crap quit pussy rape whore thief ranger rogue illusionist elementalist necromancer assassin monk cleric" ) )
		return FALSE;
...
void act( const char *format, CHAR_DATA *ch, const void *arg1, const void *arg2, int type )
{
	...
	static char * const fake_name [] ={"A grey spirit","A funny little pixie",
		"A tall, awkward man","A strange tree","Someone's arm","A withered scroll",
		"A tiny gnome","Holy Chaos","Holy Order","Demise","A murky potion",
		"Mistress Lilac","A fluffy cloud","A large worm","A mossy rock"};
...
/* Special finger output for gods (and eventually other imms) */
void finger_imm(CHAR_DATA *ch, char *argument)
{
	CHAR_DATA *fch;
	...
	if(!strcasecmp(argument,"order"))
	{
		send_to_char_color("\n\r{070}Lord Order, God of Reason and Rationality\n\r\n\r{070}Seeking to arrange all of Creation into a mosaic of neatly placed tiles,\n\rOrder is the embodiment of cold, hard logic, the personification of an\n\runchanging, unchangable reality. He seeks to lock all chance and chaos\n\rwithin an infinite, unmoving prison of Law.{300}\n\r\n\r",ch);
		if(fch == NULL)
		{
			send_to_char_color("{070}Lord Order is currently offline.{300}\n\r",ch);
		}
		else
		{
			...
			{
				send_to_char_color("{070}Lord Order is currently online.{300}\n\r",ch);
			}
		}
	}
	if(!strcasecmp(argument,"chaos"))
	{
		send_to_char_color("\n\r{020}Lord Chaos, God of Chance and Emotion\n\r\n\r{070}Within the cracks of creation Chaos works tirelessly, eternally striving\n\rto break all Law, to introduce unpredictable change and passion to\n\rCreation. Indescribable by definition, His fingers are constantly\n\rtangling the strings of Harmony, and twisting the fates of Man towards\n\rthe dark, unknown pathways.{300}\n\r\n\r",ch);
		if(fch == NULL)
		{
			send_to_char_color("{020}Lord Chaos is currently offline.{300}\n\r",ch);
		}
		else
		{
			if(IS_SET(fch->act,PLR_WIZINVIS))
			...
			{
				send_to_char_color("{020}Lord Chaos is currently online.{300}\n\r",ch);
			}
		}
	}
	if(!strcasecmp(argument,"demise"))
	{
		send_to_char_color("\n\r{010}Lord Demise, God of Death and Despair\n\r\n\r{070}Chaos and Order play the lives of Man like pieces on a gameboard.\n\rScornful of Their games, Demise lies hidden within the shadows, tempting\n\rthe fates of men to step awry, collecting the pieces as they fall. His\n\ragents attempt to hurry the souls of Man into the cold, dead embrace of\n\rtheir God.{300}\n\r\n\r",ch);
		if(fch == NULL)
		{
			send_to_char_color("{010}Lord Demise is currently offline.{300}\n\r",ch);
		}
		else
		{
			if(IS_SET(fch->act,PLR_WIZINVIS))
			...
			{
				send_to_char_color("{010}Lord Demise is currently online.{300}\n\r",ch);
			}
		}
	}
	return;
...
/* do_finger changed 02/04 to include color - Nodd */
void do_finger( CHAR_DATA *ch, char *argument)
{
	CHAR_DATA *fch;
	char const *class;
	bool loaded = FALSE;
	char buf[MAX_STRING_LENGTH], buf2[MAX_STRING_LENGTH], buf3[MAX_INPUT_LENGTH];
	char outbuf[MAX_STRING_LENGTH];
	char buf_name[MAX_STRING_LENGTH], buf_lang[MAX_STRING_LENGTH], 
	     buf_race[MAX_STRING_LENGTH], strName[MAX_STRING_LENGTH];
	char *pt;
	int length=0;
	int expire;
	int time_left,cnt;
	...
	sh_int rel_hrs;
	sh_int rel_mins;
	char pcwealth[MAX_STRING_LENGTH]; /* Emperor 11/26/99 */
	char pcage[MAX_STRING_LENGTH];
	int paccount;
	...
	if ( !strcasecmp( argument, "death" ) || !strcasecmp( argument, "althena" ) )
	{
		sprintf( buf3, "The character named '%s' cannot be found.\n\r", argument );
		send_to_char( buf3, ch );
		return;
	}
	if((!strcasecmp(argument,"chaos") || !strcasecmp(argument,"order") || !strcasecmp(argument,"demise")) && ch->level <= 95)
	{
		finger_imm(ch, argument);
		return;
	}
	if( fch==NULL)
	{
	...
	strcpy( strName, argument );
	if ( strName[0] >= 'Z' )
		strName[0] += ( 'A' - 'a' );
	/* capitalize_name( strName ); */

	/******  Information on Maiden Jordo.     Do I ever feel Rewarded.: ******/
	if( ch->vt100==0)
		length = sprintf( outbuf, " Information on %s%s%s\n\r", get_title( fch ), strName,fch->pcdata->title);
	else
		length = sprintf( outbuf, "%sInformation on %s%s%s:\n\r",ANSI_CYAN_BOLD, get_title( fch ),strName,fch->pcdata->title);
	FINGER_MODE=TRUE;
	...
	/********* Determine Class Construction **************************************************/
		sprintf(buf_lang,"%s%s:%s%2d%s %s:%s%2d%s %s:%s%2d%s %s:%s%2d%s %s:%s%2d%s %s:%s%2d%s %s:%s%2d%s %s:%s%2d", 
		ANSI_CYAN_DIM,
		/* Code below: Only classes used will be bold cyan */
		class_table[0].who_name,(fch->mclass[0] < 1) ? ANSI_WHITE_DIM : ANSI_WHITE_BOLD, fch->mclass[0],ANSI_CYAN_DIM,
		class_table[1].who_name,(fch->mclass[1] < 1) ? ANSI_WHITE_DIM : ANSI_WHITE_BOLD, fch->mclass[1],ANSI_CYAN_DIM,
		class_table[2].who_name,(fch->mclass[2] < 1) ? ANSI_WHITE_DIM : ANSI_WHITE_BOLD, fch->mclass[2],ANSI_CYAN_DIM,
		class_table[3].who_name,(fch->mclass[3] < 1) ? ANSI_WHITE_DIM : ANSI_WHITE_BOLD, fch->mclass[3],ANSI_CYAN_DIM,
		class_table[4].who_name,(fch->mclass[4] < 1) ? ANSI_WHITE_DIM : ANSI_WHITE_BOLD, fch->mclass[4],ANSI_CYAN_DIM,
		class_table[5].who_name,(fch->mclass[5] < 1) ? ANSI_WHITE_DIM : ANSI_WHITE_BOLD, fch->mclass[5],ANSI_CYAN_DIM,
		class_table[6].who_name,(fch->mclass[6] < 1) ? ANSI_WHITE_DIM : ANSI_WHITE_BOLD, fch->mclass[6],ANSI_CYAN_DIM,
		class_table[7].who_name,(fch->mclass[7] < 1) ? ANSI_WHITE_DIM : ANSI_WHITE_BOLD, fch->mclass[7]);
/*		
	sprintf( buf_lang,"Ill:%s%2d%s Ele:%s%2d%s Rog:%s%2d%s Ran:%s%2d%s Nec:%s%2d%s Cle:%s%2d%s Asn:%s%2d%s Mon:%s%2d%s",
		(fch->mclass[0] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, fch->mclass[0],ANSI_BLUE_BOLD,
		(fch->mclass[1] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, fch->mclass[1],ANSI_BLUE_BOLD,
		(fch->mclass[2] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, fch->mclass[2],ANSI_BLUE_BOLD,
		(fch->mclass[3] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, fch->mclass[3],ANSI_BLUE_BOLD,
		(fch->mclass[4] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, fch->mclass[4],ANSI_BLUE_BOLD,
		(fch->mclass[5] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, fch->mclass[5],ANSI_BLUE_BOLD,
		(fch->mclass[6] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, fch->mclass[6],ANSI_BLUE_BOLD,
		(fch->mclass[7] < 1) ? ANSI_CYAN_DIM : ANSI_CYAN_BOLD, fch->mclass[7],ANSI_BLUE_BOLD);
*/
	/********* Output name, level, class, race, and class construction ***********************/
	strcpy(buf_name, strName );
	buf_name[10]='\0';
	while(strlen(buf_name)<10)
		strcat ( buf_name, " ");
	sprintf( buf2, "%s%s %s[%s%2d %s %s%s] %s\n\r",	
		ANSI_CYAN_BOLD, capitalize(buf_name), ANSI_WHITE_DIM, ANSI_WHITE_DIM, fch->level, class, buf_race, ANSI_WHITE_DIM, buf_lang);
	...
	if (ch->ansi)
		...
		paccount = fch->pcdata->account+fch->gold;

		if ( fch->level > 96 )           strcpy(pcwealth, "Infinite");
		else if ( paccount < 10000000 )  strcpy(pcwealth, "Very Poor");
		else if ( paccount < 40000000 )  strcpy(pcwealth, "Poor");
		else if ( paccount < 90000000 )  strcpy(pcwealth, "Comfortable");
		else if ( paccount < 130000000 ) strcpy(pcwealth, "Wealthy");
		else if ( paccount < 180000000 ) strcpy(pcwealth, "Very Wealthy");
		else                             strcpy(pcwealth, "Affluent");

		if ( fch->level > 96 ) strcpy(pcage, "Eternal");
		else if ( get_age( fch ) < 25 ) strcpy(pcage, "Adolescent");
		else if ( get_age( fch ) < 40 ) strcpy(pcage, "Young");
		else if ( get_age( fch ) < 75 ) strcpy(pcage, "Adult");
		else if ( get_age( fch ) < 125 ) strcpy(pcage, "Old");
		else if ( get_age( fch ) < 175 ) strcpy(pcage, "Very Old");
		else if ( get_age( fch ) < 250 ) strcpy(pcage, "Extremely Old");
		else if ( get_age( fch ) < 425 ) strcpy(pcage, "Venerable");
		else strcpy(pcage, "Ancient");

		/* Immortals see actual values */

		if (IS_IMMORTAL(ch))
			sprintf( buf2, "%sAge: %s (%d). %sPlayed: %s%d hours. %sWealth: %s (%d).\n\r",
			ANSI_CYAN_DIM, 
			pcage, get_age(fch), 
			ANSI_CYAN_DIM, 
			ANSI_WHITE_BOLD, (int) fch->played/3600+fch->pcdata->previous_hours,
			ANSI_CYAN_DIM, 
			pcwealth, paccount );
		else
			sprintf( buf2, "%sAge: %s. %sPlayed: %s%d hours. %sWealth: %s.\n\r",
			ANSI_CYAN_DIM,
			pcage,
			ANSI_CYAN_DIM,
			ANSI_WHITE_BOLD, (int) fch->played/3600+fch->pcdata->previous_hours,
		...
		/* Old age and hours for safekeeping
		...
		*/
		...
		{
			sprintf( buf2, "%s%s has been reincarnated %s%d %stimes.\n\r",
				ANSI_CYAN_DIM,strName, ANSI_WHITE_BOLD, fch->pcdata->reincarnation, ANSI_CYAN_DIM );
			length = str_apd_max (outbuf, buf2, length, MAX_STRING_LENGTH);
		}

		expire = character_expiration(fch);

		/* Turned on expiration date for every char.. Why not? -- Merzik - 9-11-00 */

		if ( IS_IMMORTAL(ch) )
		{
			sprintf( buf2, "%sThis player file will expire in %s%d days.\n\r", ANSI_CYAN_DIM, ANSI_WHITE_BOLD,expire );
			length = str_apd_max (outbuf, buf2, length, MAX_STRING_LENGTH);
		}

		/* Old expiration code for safekeeping - Emperor 11/26/99
		...
		*/
		...
		if( which_god( fch ) != GOD_NEUTRAL)
		{
			...
			/* Removing priest/knight system for now -- Merzik - 10-8-00

			if ( fch->level == 96 && fch->sex != 2 )
			{
			switch( which_god( fch ) )
			{
			case GOD_ORDER:
			sprintf(buf, "This player is a Grand Knight of Order.\n\r");break;
			case GOD_CHAOS:
			sprintf(buf, "This player is a Grand Knight of Chaos.\n\r");break;
			default:
			sprintf(buf, "This player is a Grand Knight.\n\r");break;
			}
			...
			{
			case GOD_ORDER:
			sprintf(buf, "This player is a Grand Lady of Order.\n\r");break;
			case GOD_CHAOS:
			sprintf(buf, "This player is a Grand Lady of Chaos.\n\r");break;
			default:
			sprintf(buf, "This player is a Grand Lady.\n\r");break;
			}
			length = str_apd_max (outbuf, buf, length, MAX_STRING_LENGTH);
			}

			if ( fch->level == 97 && fch->sex != 2)
			{
			switch( which_god( fch ) )
			{
			case GOD_ORDER:
			sprintf(buf, "This player is a Priest of Order.\n\r");break;
			case GOD_CHAOS:
			sprintf(buf, "This player is a Priest of Chaos.\n\r");break;
			default:
			sprintf(buf, "This player is a Priest of the Gods.\n\r");break;
			}
			length = str_apd_max (outbuf, buf, length, MAX_STRING_LENGTH);
			}
			...
			{
			case GOD_ORDER:
			sprintf(buf, "This player is a Priestess of Order.\n\r");break;
			case GOD_CHAOS:
			sprintf(buf, "This player is a Priestess of Chaos.\n\r");break;
			default:
			sprintf(buf, "This player is a Priestess of the Gods.\n\r");break;
			}
			length = str_apd_max (outbuf, buf, length, MAX_STRING_LENGTH);
			}

			if ( fch->level == 98 && fch->sex != 2)
			{
			switch( which_god( fch ) )
			{
			case GOD_ORDER:
			sprintf(buf, "This player is a High Priest of Order.\n\r");break;
			case GOD_CHAOS:
			sprintf(buf, "This player is a High Priest of Chaos.\n\r");break;
			default:
			sprintf(buf, "This player is a High Priest of the Gods.\n\r");break;
			}
			length = str_apd_max (outbuf, buf, length, MAX_STRING_LENGTH);
			}
			...
			{
			case GOD_ORDER:
			sprintf(buf, "This player is a High Priestess of Order.\n\r");break;
			case GOD_CHAOS:
			sprintf(buf, "This player is a High Priestess of Chaos.\n\r");break;
			default:
			sprintf(buf, "This player is a High Priestess of the Gods.\n\r");break;
			}
			length = str_apd_max (outbuf, buf, length, MAX_STRING_LENGTH);
			}
			...
			*/
			if ( fch->level < 99 )
			{
				...
				case GOD_ORDER:
					sprintf(buf, "%sThis player follows %sOrder.\n\r",ANSI_CYAN_DIM,ANSI_WHITE_BOLD);break;
				case GOD_CHAOS:
					sprintf(buf, "%sThis player follows %sChaos.\n\r",ANSI_CYAN_DIM,ANSI_YELLOW_BOLD);break;
				case GOD_DEMISE:
					sprintf(buf, "%sThis player follows %sDemise.\n\r",ANSI_CYAN_DIM,ANSI_RED_BOLD);break;
				case GOD_POLICE:
					sprintf(buf, "%sThis player is an %sEnforcer of the Realms.\n\r",ANSI_CYAN_DIM,ANSI_BLUE_BOLD);break;
				case GOD_CREATOR:
					sprintf(buf, "%sThis player is an %sArea Creator.\n\r",ANSI_CYAN_DIM,ANSI_RED_BOLD);break;
				default:
					sprintf(buf, "%sBUG: unknown God type.\n\r",ANSI_WHITE_BOLD);break;
				}
				length = str_apd_max (outbuf, buf, length, MAX_STRING_LENGTH);
			}
		...
		if( IS_SET( fch->pcdata->player2_bits, PLR2_TANK ) )
		{
			if ( fch->sex == 2 )
				sprintf( buf, "%sThis player has dedicated herself to teaching others.\n\r",ANSI_CYAN_DIM );
			else
				sprintf( buf, "%sThis player has dedicated himself to teaching others.\n\r",ANSI_CYAN_DIM );
			length = str_apd_max( outbuf, buf, length, MAX_STRING_LENGTH );
		}
		...
		if( IS_SET( fch->act, PLR_OUTCAST) )
		{
			...
			if ( fch->pcdata->clan)
			{
				if ( !strcasecmp( fch->name, fch->pcdata->clan->leader ) )
					...
					strName, ANSI_WHITE_BOLD, fch->pcdata->clan_name);
				else
					if ( !strcasecmp( fch->name, fch->pcdata->clan->number1 )
						...
						sprintf(buf, "%s%s%s is a leader of %s%s.\n\r", ANSI_CYAN_DIM, get_title(fch),
						strName, ANSI_WHITE_BOLD,fch->pcdata->clan_name);
					else
						sprintf(buf, "%s%s%s is a member of %s%s.\n\r", ANSI_CYAN_DIM, get_title(fch),
						strName, ANSI_WHITE_BOLD,fch->pcdata->clan_name);
				length = str_apd_max( outbuf, buf, length, MAX_STRING_LENGTH);
			}
			...
			if( fch->pcdata->arrested == TRUE )
			{
				...
				if(time_left > 0)
				{
					...
					if(rel_mins == 0)
						rel_mins = 1;
					sprintf(buf2, "%sImprisoned in the dungeons of the realms for%s %d days, %d hours, %d minutes.\n\r",
							ANSI_CYAN_BOLD,ANSI_WHITE_BOLD,rel_days, rel_hrs, rel_mins);
					length = str_apd_max (outbuf, buf2, length, MAX_STRING_LENGTH);
				}
			}

			/* Moving mob deaths from finger to score -- Merzik - 9-11-00 */

			if( which_god( fch )==GOD_CHAOS || which_god(fch)==GOD_ORDER ||
				which_god( fch )==GOD_DEMISE || which_god(fch)==GOD_POLICE )
				sprintf( buf2, "%s%s's standings:\n\r%sNeutrals Dead: %s%3d    %sNeutrals that have killed %s: %s%3d\n\r%sEnemies Dead:  %s%3d    %sEnemies that have killed %s:  %s%3d\n\r",

				/*
				Monsters that have killed %s: %3d\n\r",
				*/
				ANSI_CYAN_DIM,
				strName,
				ANSI_CYAN_DIM,ANSI_WHITE_BOLD,fch->pcdata->history[HISTORY_KILL_PC],ANSI_CYAN_DIM,
				strName,ANSI_WHITE_BOLD,
				fch->pcdata->history[HISTORY_KILL_BY_PC],
				ANSI_CYAN_DIM,ANSI_WHITE_BOLD,
				fch->pcdata->history[HISTORY_KILL_EN],ANSI_CYAN_DIM,
				strName,ANSI_WHITE_BOLD,
			...
			*/
				...
				sprintf( buf2, "%s%s's standings:\n\r%sPlayers Dead: %s%3d    %sPlayers that have killed %s: %s%3d\n\r",
				/*
				Monsters that have killed %s: %3d\n\r",
				*/
				ANSI_CYAN_DIM,
			...
			*/
			...
			if ( fch->pcdata->army_status > 0 )
			{
				if ( fch->pcdata->army_status == 1 )
					sprintf( buf2, "%sRace Army Status: %sActive   %sKills: %s%3d\n\r", 
							ANSI_CYAN_DIM,ANSI_WHITE_BOLD,ANSI_CYAN_DIM,ANSI_WHITE_BOLD,fch->pcdata->history[HISTORY_KILL_ARMY] );
				else
					sprintf( buf2, "%sRace Army Status: %sRetired  %sKills: %s%3d\n\r", 
							ANSI_CYAN_DIM,ANSI_WHITE_DIM,ANSI_CYAN_DIM,ANSI_WHITE_DIM,fch->pcdata->history[HISTORY_KILL_ARMY] );
				length = str_apd_max ( outbuf, buf2, length, MAX_STRING_LENGTH );
			}

			if ( fch->pcdata->clan_army > 0 )
			{
				if ( fch->pcdata->clan_army == 1 )
					sprintf( buf2, "%sClan Army Status: %sActive   %sKills: %s%3d\n\r", 
							ANSI_CYAN_DIM,ANSI_WHITE_BOLD,ANSI_CYAN_DIM,ANSI_WHITE_BOLD,fch->pcdata->history[HISTORY_KILL_CLAN] );
				else
					sprintf( buf2, "%sClan Army Status: %sRetired  %sKills: %s%3d\n\r", 
							ANSI_CYAN_DIM,ANSI_WHITE_DIM,ANSI_CYAN_DIM,ANSI_WHITE_DIM,fch->pcdata->history[HISTORY_KILL_CLAN] );
				length = str_apd_max ( outbuf, buf2, length, MAX_STRING_LENGTH );
			}
			...
			{
				sprintf( buf2, "%sInternet Email address: %s\n\r", ANSI_CYAN_DIM,fch->pcdata->mail_address);
				length = str_apd_max ( outbuf, buf2, length, MAX_STRING_LENGTH);
			}

			/* Disabling html home page - abuse of color by players, and
			nobody ever really uses it anyway -- Merzik - 9-11-00
			...
			*/
			...
			{
				sprintf(buf2,"%sHas a castle that cost $%d000 to build.\n\r",
					ANSI_CYAN_DIM,fch->pcdata->castle->cost);
				length = str_apd_max (outbuf, buf2, length, MAX_STRING_LENGTH);
			}
			...
			if( ch->level>96 || which_god(ch)==GOD_POLICE )
			{
				...
				{
					sprintf( buf2, "%sTotal practices given: %s%d\n\r",
						ANSI_CYAN_DIM,ANSI_WHITE_BOLD,fch->pcdata->give_prac_pos );
					length = str_apd_max (outbuf, buf2, length, MAX_STRING_LENGTH);
				}
				if( fch->pcdata->give_prac_neg > 0)
				{
					sprintf( buf2, "%sTotal practices taken: %s%d\n\r",
						ANSI_CYAN_DIM,ANSI_WHITE_BOLD,fch->pcdata->give_prac_neg );
					length = str_apd_max ( outbuf, buf2, length, MAX_STRING_LENGTH);
				}

				/* This is annoying - Emperor
			...
			/* Added fixes for the double 'Last time in the realms' message when
			player reconnects or goes linklost -- Merzik - 10-16-00 */
				...
				|| (is_affected(fch, gsn_greater_stealth) && !(IS_SET(ch->act, PLR_HOLYLIGHT) || (is_affected(ch, gsn_truesight) && ch->mclass[CLASS_ILLUSIONIST] >= fch->level) )))
			{
				...
				if(loaded)
				{
					if (IS_IMMORTAL(ch) || ch->which_god == GOD_POLICE)
					{
						sprintf( buf2, "%sInternet computer last on: %s\n\r", ANSI_CYAN_DIM,fch->desc->old_host );
					}
					else
					{
						...
						if ( IS_IMMORTAL(ch) || ch->which_god == GOD_POLICE )
						{
							if ( strcasecmp( fch->name, "chaos" ) && strcasecmp( fch->name, "order" ) )
							{
								if ( fch->desc->old_host != NULL && *fch->desc->old_host != '\0' )
									...
									sprintf( buf2, "%sInternet computer last on: \n\r",ANSI_CYAN_DIM );
							}
						}
						else
						{
							if ( strcasecmp( fch->name, "chaos" ) && strcasecmp( fch->name, "order" ) )
							{
								if ( fch->desc->old_domain != NULL && *fch->desc->old_domain != '\0' )
									...
									sprintf( buf2, "%sInternet domain last on: \n\r",ANSI_CYAN_DIM );
							}
						}
					}
					if ( strcasecmp( fch->name, "chaos" ) && strcasecmp( fch->name, "order" ) )
				...
				if( is_desc_valid( fch ) )
				{
					...
					{
						if ( strcasecmp( fch->name, "chaos" ) && strcasecmp( fch->name, "order" ) )
							sprintf( buf2, "%sInternet computer on: %s\n\r", ANSI_CYAN_DIM,fch->desc->host );
					}
					else
					{
						if ( strcasecmp( fch->name, "chaos" ) && strcasecmp( fch->name, "order" ) )
							sprintf( buf2, "%sInternet domain on: %s\n\r", ANSI_CYAN_DIM,fch->desc->domain );
					}
