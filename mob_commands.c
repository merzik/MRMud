...
void do_mpechoat( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char_color( argument, victim );
...
void do_mpmload( CHAR_DATA *ch, char *argument )
{
	...
	/* If char is fighting and room is safe, somebody's trying to cheat... -- Merzik */
	if ( ch->fighting != NULL && IS_SET( ch->in_room->room_flags, ROOM_SAFE ) )
		return;
...
/* Lets creators purge a mob out of the room by using its vnum */

void do_mpmobpurge( CHAR_DATA *ch, char *argument )
{
	int victim_vnum;
	...
	one_argument( argument, arg );
	victim_vnum = is_number( arg ) ? atol( arg ) : -1 ;

	if ( victim_vnum < 1 || victim_vnum > MAX_VNUM )
		return;
	...
	for ( victim = ch->in_room->first_person; victim != NULL; victim = vnext )
	{
		vnext = victim->next_in_room;
		if ( IS_NPC( victim ) && victim != ch )
			if ( victim->pIndexData->vnum == victim_vnum )
			{
				extract_char( victim, TRUE );
				return;
			}
...
void do_mpgoto( CHAR_DATA *ch, char *argument )
{
	...
	if ( !IS_NPC( ch ) )
	{
		...
		sprintf( strLog, "%-12s Using: Mpgoto Arg: %s", ch->name, argument );
		log_quiet( strLog );
	}
...
void do_mpforce( CHAR_DATA *ch, char *argument )
{
	...
	{
		char strLog[MAX_INPUT_LENGTH];
		sprintf( strLog, "%-12s Using: Mpforce Arg: %s", ch->name, argument );
		log_string( strLog );
	}
...
/* Used in area files to catch users of discovered cheats -- Merzik 6-25-01 */

void do_mpcheatlog( CHAR_DATA *ch, char *argument )
{
	char buf[MAX_INPUT_LENGTH];
	char arg[MAX_INPUT_LENGTH];
	CHAR_DATA *victim;
	...
	sprintf( buf, "**CHEAT** %s logging %s in room %d: %s", ch->name, victim->name, ch->in_room->vnum, argument );
	log_string( buf );
...
void do_mpmset( CHAR_DATA *ch, char *argument )
{
	...
	if ( ( victim = get_char_room_no_illusion( ch, arg1 ) ) == NULL )
	{
		return;
	}
	...
	*/
	...
	if ( !strcasecmp( arg2, "quest" ) )
	{
		...
		{
			send_to_char("You must use mpmset questr instead.\n\r", ch);
			return;
		}
		...
		else
		{
			if (IS_NPC(ch))
			{
				/* Changing this to only set bits from the mob's home area -- Merzik 12/19/2003 */
