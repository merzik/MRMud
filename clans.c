...
void fread_clan( CLAN_DATA *clan, FILE *fp )
{
	...
	for ( ; ; )
	{
		...
		case '*':
			...
			/* Adding bamfin/bamfout stuff - Emperor */

		case 'B':
			KEY( "Bamfin", clan->bamfin, fread_string( fp ) );
			KEY( "Bamfout", clan->bamfout, fread_string( fp ) );
...
bool load_clan_file( char *clanfile )
{
	...
	clan->bamfin = STRALLOC( "" );
	clan->bamfout = STRALLOC( "" );
	clan->bank = 0;
...
void load_clans( )
{
	...
	fclose( fpList );
	log_string("Clans loaded successfully." );
	return;
}
...
void do_initiate( CHAR_DATA *ch, char *argument )
{
	...
	act( "$n teaches $N in the ways of $t.", ch, clan->name, victim, TO_NOTVICT );
	act( "$n teaches you in the ways of $t.\n\rYou must pledge your support to $t to become a full member.", ch, clan->name, victim, TO_VICT );
...
void do_renounce( CHAR_DATA *ch, char *argument )
{
	...
	if( check_password(ch, argument) )
	{
		...
		/* Retire from clan army if they're currently active
		-- Causes problems when moving clans
...
void do_orders( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_IMMORTAL(ch))
	{
		...
		/* Removing pkill data from ORDERS command - Ken 09/10/99
...
void do_clans( CHAR_DATA *ch, char *argument )
{
	...
	if ( argument[0] == '\0' )
	{
		if ( ch->level > 96 )/* Admin player output */
		{
			send_to_char_color( "{110}\n\rClan          Founder      Members        Coffers    Tax  Bdoors  Home\n\r", ch);
			send_to_char_color( "{110}_____________________________________________________________________________\n\r\n\r", ch );
			for ( clan = first_clan; clan; clan = clan->next )
			{
				if (clan->type != CLAN_WARLIKE)
					continue;
				ch_printf_color( ch, "{070}%-13s %-10s{010}     %-7d  %12d  %3d%%    %2d    %5d\n\r",
					clan->name, clan->leader, clan->members, clan->coffers, clan->tax, clan->num_backdoors, clan->home );
			...
			if ( !count )
				...
				send_to_char_color( "{110}_____________________________________________________________________________\n\r\n\rUse 'clan <clan>' for detailed information on a specific clan.\n\r", ch );
			return;
		}

		else/* Normal player output */
		{
			send_to_char_color( "{110}\n\rClan          Founder            Members\n\r", ch);
			send_to_char_color( "{110}_________________________________________\n\r\n\r", ch );
			for ( clan = first_clan; clan; clan = clan->next )
			{
				if (clan->type != CLAN_WARLIKE)
					continue;
				ch_printf_color( ch, "{070}%-13s %-13s{010}      %-7d\n\r",
					clan->name, clan->leader, clan->members);

				/* Removing clan pkill histories - Ken 09/10/99
				*
				* clan->pkills[0]+clan->pkills[1]+
	...
	/* Removing clan kill data from CLAN command - Ken 09/10/99
	Blah... -- Merzik */
	...
	ch_printf( ch, "   Order Kills : %4d    Order Deaths : %4d\n\r", clan->pkills[CLAN_ORDER_KILL], clan->pdeaths[CLAN_ORDER_DEATH]);
	ch_printf( ch, "    Race Kills : %4d     Race Deaths : %4d\n\r", clan->pkills[CLAN_RACE_KILL], clan->pdeaths[CLAN_RACE_DEATH]);
	ch_printf( ch, "    Army Kills : %4d     Army Deaths : %4d\n\r", clan->pkills[CLAN_ARMY_KILL], clan->pdeaths[CLAN_ARMY_DEATH] );
	...
	*/
		...
		|| IS_IMMORTAL(ch) )
	{
		...
		ch_printf( ch, "Number of Back Doors: %d\n\r", clan->num_backdoors);
		ch_printf( ch, "Bank: %d\n\r", clan->bank );
		ch_printf_color( ch, "Bamfin: %s\n\r", clan->bamfin );
		ch_printf_color( ch, "Bamfout: %s\n\r", clan->bamfout );
	}
	if (IS_IMMORTAL(ch))
	{
		ch_printf( ch, "Home: %6d  Store: %6d  Guard: %6d  Healer: %6d  Founder Vnum: %d\n\r",
...
void do_rent( CHAR_DATA *ch, char *argument )
{
	...
	if (clan->bank>0)
	{
		ch_printf(ch, "Rent for Banks : %d\n\r", clan->bank*RENT_BANK );
		rent+=RENT_BANK;
	}
	ch_printf(ch, "\n\rTotal is : %d gold.\n\r", rent);
...
void do_gohome( CHAR_DATA *ch, char *argument )
{
	...
	/* Clan leaders can customize gohome - Emperor 11/26/99 */

	act( "$n $T", ch, NULL, (ch->pcdata && ch->pcdata->clan->bamfout[0] != '\0')
		? ch->pcdata->clan->bamfout : "turns sideways and disappears.", TO_ROOM );
	...
	*/
	...
	act( "$n $T", ch, NULL, (ch->pcdata && ch->pcdata->clan->bamfin[0] != '\0')
		? ch->pcdata->clan->bamfin : "appears in the room.", TO_ROOM );
...
void do_clanwhere( CHAR_DATA *ch, char *argument )
{
	...
	if ( IS_NPC( ch ) || ( !ch->pcdata->clan && ( ch->level < 99 ) ) )
	{
		send_to_char( "Huh?\n\r", ch );
		return;
	}
	...
	sprintf( buf3, "Locations of members of Clan %s:\n\r", clan->name );
	length = str_cpy_max( buf, buf3, MAX_STRING_LENGTH);

	for( fch=first_char; fch!=NULL; fch=fch->next)
	{
		if (!IS_NPC(fch) && fch->pcdata->clan !=NULL
			&& fch->pcdata->clan == clan)
		{
			sprintf(buf1, "%-13s in %-20s ", fch->name, fch->in_room->name);
			buf1[38]='\0';
			length = str_apd_max( buf, buf1, length, MAX_STRING_LENGTH);

			/* Adding IP address -- Merzik - 10-23-00 */

			if ( fch->desc != NULL )
				if ( ch->level > 98 )
					sprintf( buf2, " %-15s", fch->desc->host );
				else
					sprintf( buf2, " %-15s", fch->desc->domain );
			else
				sprintf( buf2, " Link lost " );

			buf2[16] = '\0';

			sprintf(buf1, " %-18s", fch->in_room->area->name );
			buf1[19]='\0';

			length = str_apd_max( buf, buf1, length, MAX_STRING_LENGTH);
			length = str_apd_max( buf, buf2, length, MAX_STRING_LENGTH);
...
/*
* Added for admins to have an easier way to check for multi-loggers
* Merzik - 11-25-00
*/
...
void do_clanlist( CHAR_DATA *ch, char *argument )
{
	...
	sprintf( buf3, "List of members of Clan %s:\n\r", clan->name );
	length = str_cpy_max( buf, buf3, MAX_STRING_LENGTH);

	for( fch=first_char; fch!=NULL; fch=fch->next)
	{
		if ( !IS_NPC( fch ) && fch->pcdata->clan !=NULL && fch->pcdata->clan == clan )
		{
			sprintf( buf1, "%-20s", fch->name );
...
void do_makeclan( CHAR_DATA *ch, char *argument )
{
	...
	/*
	* Spaces in the file name cause problems with file system - replacing
	* with an underscore -- Merzik 12-27-00
	*/
	...
	clan->num_leaders = 0;
	clan->bank = 0;
	clan->bamfin = STRALLOC( "appears through a portal." );
	clan->bamfout = STRALLOC( "turns sideways and disappears." );
...
void destroy_clan(CLAN_DATA *deadclan)
{
	...
	if ( room != NULL )
		...
		log_printf( "Null home room found while deleting clan %s", deadclan->name );
...
void do_setclan( CHAR_DATA *ch, char *argument )
{
	...
	if ( arg1[0] == '\0' )
	{
		if (IS_IMMORTAL(ch))
		{
			...
			send_to_char( "\n\rField being one of:\n\r", ch );
			send_to_char( " leader number1 number2 number3 number4 members home store bamfin\n\r", ch);
			send_to_char( " obj1 obj2 guard healer motto desc email founder type bamfout backdoors\n\r", ch );
		}
		else send_to_char( "Usage: setclan <motto|desc|email|tax|storeroom|bamfin|bamfout>\n\r", ch );
		return;
	}
	...
	if (( IS_IMMORTAL (ch) &&!strcasecmp( arg2, "bamfin" ) )
		|| !strcasecmp( arg1, "bamfin" ) )
	{
		smash_tilde( argument );
		STRFREE(clan->bamfin );
		clan->bamfin = str_dup( argument );
		send_to_char( "Ok.\n\r", ch );
		save_clan( clan );
		return;
	}

	if (( IS_IMMORTAL (ch) &&!strcasecmp( arg2, "bamfout" ) )
		|| !strcasecmp( arg1, "bamfout" ) )
	{
		smash_tilde( argument );
		STRFREE(clan->bamfout );
		clan->bamfout = str_dup( argument );
		send_to_char( "Ok.\n\r", ch );
		save_clan( clan );
		return;
	}
	...
	*/
	...
	if ( ch->level > 98 && !strcasecmp( arg2, "coffers" ) )
	{
		if ( atoi( argument ) < 0 || atoi( argument ) > 2000000000 )
		{
			send_to_char( "Invalid number value.\n\r", ch );
			return;
		}
		clan->coffers = atoi( argument );
		send_to_char( "Done.\n\r", ch );
		save_clan( clan );
		return;
	}

	if ( ch->level > 98 && !strcasecmp( arg2, "backdoors" ) )
	{
		if ( atoi( argument ) < 0 )
		{
			send_to_char( "Invalid number value.\n\r", ch );
...
void do_clantalk (CHAR_DATA * ch, char *argument)
{
	CHAR_DATA *fch;
	PLAYER_GAME *fpl;
	char buf[MAX_STRING_LENGTH], jbuf[MAX_STRING_LENGTH];

	if (IS_IMMORTAL(ch))
	{
		do_immclantalk(ch, argument);
		return;
	}
	...
	if ( argument[0] == '\0' )
	{
		send_to_char ( "Clantalk what?\n\r", ch );
		return;
	}

	sprintf ( jbuf, "You clantalk '%s'", argument );
	if ( ch->ansi == 1 )
		sprintf ( buf, "\033[1;%d;%dm%s\n\r", ch->pcdata->color[0][COLOR_CLANTALK],
		ch->pcdata->color[1][COLOR_CLANTALK], justify( jbuf ) );
	else
		sprintf ( buf, "%s\n\r", justify( jbuf ) );
	send_to_char ( buf, ch );

	sprintf (buf, "%s clantalks '%s'", get_name (ch), argument);
	...
	for (fpl = first_player; fpl != NULL; fpl = fpl->next)
	{
		...
		if ( ch->pcdata->clan != fch->pcdata->clan
			&& ch->pcdata->clan != fch->pcdata->clan_listen )
			continue;

		if ( !IS_NPC( fch ) && fch->ansi == 1 )
			sprintf ( buf, "\033[1;%d;%dm%s\n\r", fch->pcdata->color[0][COLOR_CLANTALK],
			fch->pcdata->color[1][COLOR_CLANTALK], jbuf );
		else
			sprintf ( buf, "%s\n\r", jbuf );
...
void do_listen( CHAR_DATA *ch, char *argument )
{
	...
	if ( argument[0] == '\0' )
	{
		if ( ch->pcdata->clan_listen == NULL )
		{
			send_to_char( "You are currently not listening to any clan.\n\r", ch );
			return;
		}
		else
		{
			ch_printf( ch, "Currently listening to Clan %s.\n\r", ch->pcdata->clan_listen->name );
			return;
		}
	}
	...
	if ( !strcasecmp( argument, "clear" ) )
	{
		ch->pcdata->clan_listen = NULL;
		send_to_char( "Listen flag cleared.\n\r", ch );
		return;
	}
	...
	{
		ch->pcdata->clan_listen = clan;
		ch_printf( ch, "Listen flag set to Clan %s.\n\r", clan->name );
		return;
	}
	return;
}
...
void do_immclantalk (CHAR_DATA * ch, char *argument)
{
	...
	if ( ch->pcdata->clan_listen == NULL )
	{
		send_to_char( "You are not listening to a clan!\n\r", ch );
		return;
	}
	...
	for (fpl = first_player; fpl != NULL; fpl = fpl->next)
	{
		...
		if ( ch->pcdata->clan_listen != fch->pcdata->clan
			&& ch->pcdata->clan_listen != fch->pcdata->clan_listen )
			continue;
...
/*
Consolidating all of the clan rent code into this function
for easier maintenance. Replaced the code snippets in
update.c and act_wiz.c with calls to this function.
-- Merzik 1/7/2003
*/
...
void collect_rent()
{
	...
	for (clan = first_clan; clan; clan = next_clan)
	{
		next_clan = clan->next;
		if (clan->type == CLAN_PEACEFUL)
			lRent = RENT_BASIC_ORDER_HALL;
		else
			lRent = RENT_BASIC_CLAN_HALL;

		lRent += RENT_PER_GUARD * clan->num_guards;
		lRent += RENT_PER_HEALER * clan->num_healers;
		lRent += RENT_PER_BACKDOOR * clan->num_backdoors;
		lRent += RENT_BANK * clan->bank;

		/* Temporary reduction */
		lRent /= 10;

		if (clan->coffers - lRent > 0)
		{
			clan->coffers -= lRent;
			sprintf(strBuffer, "You paid %ld in rent. Coffers now at %ld.\n\r", lRent, clan->coffers);
			log_printf("Clan %s paid %ld in rent. Their coffers now at %ld.", clan->name, lRent, clan->coffers);
			send_clan_message (clan, strBuffer);
			continue;
		}
		else
		{
			/* Disabling for now since healers/guards do not exist -- Merzik 1-7-2003
			...
			{
			clan->coffers -= lRent;
			sprintf(strBuffer, "Your healers were all fired to cover costs.\n\rYou paid %ld in rent. Coffers now at %ld.\n\r", lRent, clan->coffers);
			log_printf("Clan %s paid %ld in rent after firing their healers. Their coffers now at %ld.", clan->name, lRent, clan->coffers);
			send_clan_message (clan, strBuffer);
			continue;
			}
			}
			...
			delete_mob(oldguard);
			lRent -= RENT_PER_GUARD * clan->num_healers;
			...
			{
			clan->coffers -= lRent;
			sprintf(strBuffer, "Your guards were all fired to cover costs.\n\rYou paid %ld in rent. Coffers now at %ld.\n\r", lRent, clan->coffers);
			log_printf("Clan %s paid %ld in rent after firing their guards. Their coffers now at %ld.", clan->name, lRent, clan->coffers);
			send_clan_message (clan, strBuffer);
			continue;
			}
			}
			*/
			if (clan->num_backdoors > 0)
			{
				log_printf("Removing backdoors of Clan %s...", clan->name);
				ROOM_INDEX_DATA *room = NULL;
				int vnum, door;
				for (vnum = 20000; vnum < 24500; vnum++)
				{
					if ((room = get_room_index(vnum)) != NULL
						&& IS_SET (room->room_flags, ROOM_IS_CASTLE)
						&& room->creator_pvnum == clan->founder_pvnum)
					{
						log_printf("Scanning room %d for backdoors...", vnum);
						for (door = 0; door <= 5; door++)
							if (room->exit[door] && IS_SET(room->exit[door]->exit_info, EX_CLAN_BACKDOOR))
							{
								log_printf("Removing door %d...", door);
								STRFREE (room->exit[door]->description);
								STRFREE (room->exit[door]->keyword);
								DISPOSE (room->exit[door]);
								top_exit--;
								log_string("Door removed!");
							}
					}
				}
				lRent -= RENT_PER_BACKDOOR * clan->num_backdoors;
				...
				{
					clan->coffers -= lRent;
					sprintf(strBuffer, "Your backdoors were repossessed to cover costs.\n\rYou paid %ld in rent. Coffers now at %ld.\n\r", lRent, clan->coffers);
					log_printf("Clan %s paid %ld in rent after having their backdoors repossessed. Their coffers now at %ld.", clan->name, lRent, clan->coffers);
					send_clan_message(clan, strBuffer);
					continue;
				}
			}
			...
			sprintf(strBuffer, "The clan of %s has been disbanded due to lack of adequate funding!", clan->name);
			do_echo(NULL, strBuffer);
			destroy_clan(clan);
		}
	}
	/* Save castles in case we dumped any backdoors */
	castle_needs_saving = TRUE;
	save_notes();
	save_all_clans();
	return;
