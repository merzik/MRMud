...
void do_give( CHAR_DATA *ch, char *argument )
{
	...
	if ( is_number( arg1 ) )
	{
		...
		if ( ( victim = get_char_room_no_illusion( ch, arg2 ) ) == NULL )
		{
		...
		if( amount> victim->level*victim->level*100 && !IS_NPC(victim))
		{
			send_to_char( "You cannot give that much gold to that level of a character.\n\r", ch);
			amount=victim->level*victim->level*100;
		}
			...
			if( get_trust( ch ) < MAX_LEVEL && !IS_NPC(ch))
				...
				if( !IS_NPC( victim ) )
					if( IS_NPC( ch) || (!IS_NPC( ch) && IS_IMMORTAL(ch)))
						...
						* Admin should not be able to re-engrave equipment
						* -- Merzik 11-13-00
						*/

						if( !IS_NPC( ch ) && !IS_NPC( victim ) && IS_IMMORTAL( ch ) && obj->owned_by == '\0' )
							obj->owned_by = victim->pcdata->pvnum;
			  ...
			  bool fDisplay )
{
	...
	if((obj2=get_obj_wear_vnum(ch,obj->pIndexData->vnum))!=NULL)
		...
		if(obj->owned_by!=0)
			...
			if ((location==ITEM_WEAR_FINGER||location==-1)&& CAN_WEAR( obj, ITEM_WEAR_FINGER ))
			{
			...
			if ((location==ITEM_WEAR_SHIELD||location==-1)&& CAN_WEAR( obj, ITEM_WEAR_SHIELD ) )
			{
				...
				if ( !remove_obj( ch, WEAR_SHIELD, fReplace , fDisplay) )
					return FALSE;
				if( !IS_NPC( ch ) )
					if( ch->class == 0 || ch->class == 6 || ch->class == 4 || ch->class == 7 )
						/* || ch->class==5) */
					{
						if( fDisplay )
							send_to_char( "You cannot use that kind or armor.\n\r", ch);
...
void do_buy( CHAR_DATA *ch, char *argument )
{
	...
	else
	{
		...
		if ( time_info.hour < keeper->pIndexData->pShop->open_hour )
		{
			...
			/* Added open times to shops -- Merzik - 9-5-00 */

			if ( keeper->pIndexData->pShop->open_hour == 0 )
			{
				sprintf( buf, "The shop appears to be closed. Try back at midnight.\n\r" );
				send_to_char( buf, ch );
				return ;
			}

			if ( keeper->pIndexData->pShop->open_hour == 12 )
			{
				sprintf( buf, "The shop appears to be closed. Try back at noon.\n\r" );
				send_to_char( buf, ch );
				return ;
			}
			...
			if ( keeper->pIndexData->pShop->open_hour > 12 )
			{
				sprintf( buf, "The shop appears to be closed. Try back at %dpm.\n\r", ( keeper->pIndexData->pShop->open_hour - 12 ) );
				send_to_char( buf, ch );
				return ;
			}
			...
			if ( keeper->pIndexData->pShop->open_hour < 12 )
			{
				sprintf( buf, "The shop appears to be closed. Try back at %dam.\n\r", keeper->pIndexData->pShop->open_hour );
				send_to_char( buf, ch );
				return ;
			}
...
void do_engrave( CHAR_DATA *ch, char *argument )
{
	...
	case ITEM_WEAPON:
		...
		break;
	case ITEM_CONTAINER:
		break; /* Just so we don't get kicked out for containers -- Merzik */
	default:
	...
	if( obj->item_type != ITEM_CONTAINER ) /* No affect added to containers */
	{
...
void do_make_flash( CHAR_DATA *ch, char *argument )
{
	...
	if ( cnt == -1 || ch->class != CLASS_ASSASSIN || ch->mclass[CLASS_ASSASSIN] != ch->level )
	{
