...
void save_char_obj(CHAR_DATA *ch, int which_type)
{
	...
	if(which_type == NORMAL_SAVE)
	{
		...
		*/
		strcpy(strsave, get_player_filename(cap_name, PFILE_TYPE_NORMAL));
		strcpy(strtemp, get_player_filename(cap_name, PFILE_TYPE_TEMP));
	}
	else
	{
		/*
		...
		*/
		strcpy(strsave, get_player_filename(cap_name, PFILE_TYPE_BACKUP));
		strcpy(strtemp, get_player_filename(cap_name, PFILE_TYPE_BACKUPTEMP));
	}
	/* save char to "temp.name" then rename to "name" after all done for safety */

	game_time_2=get_game_usec();

	/* No use for log spamming...

	log_printf("save_char_obj: strsave is %s", strsave);
	log_printf("save_char_obj: strtemp is %s", strtemp);
	*/

	remove( strtemp );

	/* log_printf("save_char_obj: strtemp is now %s", strtemp); */

	fp = fopen( strtemp, "w" ) ;

	/* log_printf("save_char_obj: fp is %s", fp); */
...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
	...
	fprintf( fp, "Clanarmy %d\n", ch->pcdata->clan_army );
	fprintf( fp, "Tank_change %d\n", ch->pcdata->tank_change );
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name )
{
	...
	*ch->pcdata = pcdata_zero;
	...
	ch->pcdata->pfile_version = 0;
	ch->pcdata->password = str_dup( "" );
	...
	if( d->descriptor != -999 )
	{
		...
		*(buf+2)='\0';
		...
		ch->pcdata->channel = 0;
		ch->pcdata->pkill_type = KILL_NONE;
	...
	/* if( ch->pcdata->player_fp == NULL ) */
	{
		...
		*/
		strcpy(strsave, get_player_filename(name, PFILE_TYPE_NORMAL));
	...
	* if( ch->pcdata->pvnum != 166 &&
	* ch->pcdata->pvnum != 107 )
	* ch->pcdata->pvnum != 212408
	* {
	...
	*/
	...
	/* no HOLYLIGHT except for imms - Emperor 1/00 */
	/*
	if( IS_SET(ch->act, PLR_HOLYLIGHT ) && ch->level<MAX_LEVEL-2 )
...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
	...
	for ( ; ; )
	{
		...
		case 'C':
			...
			KEY( "Clanarmy", ch->pcdata->clan_army, fread_number( fp ) );
		...
		case 'T':
			...
			KEY( "Tank_change", ch->pcdata->tank_change, fread_number( fp ) );
		...
		case 'V':
			...
			KEY( "Version", ch->pcdata->pfile_version, fread_number(fp));
...
void fwrite_poison_data( POISON_DATA *pd, FILE *fp )
{
	...
	fprintf( fp, "POISON_DATA 1 %d %d %d %d %d %d %d %ld %ld\n",
		pd->for_npc?1:0,
		(int)pd->poison_type,
...
void save_notes()
{
	...
	for (pnote = first_note; pnote->next != NULL; pnote = pnote_next)
	{
		...
		iCurTime = current_time - (24 * 60 * 60 * 28); /* Now four weeks old */
		if (pnote->time <= iCurTime)/* Delete the note */
		{
#ifdef UNLINKCHECK
			{
				NOTE_DATA *tnote;
	...
	fflush(fp);
	fclose(fp);
	/* log_string("Notes saved successfully."); */
	return;
}
