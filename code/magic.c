...
bool saves_spell(int level, CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	/* Gnomes have advanced resistance to magic - 15% bonus -- Merzik 6-15-01 */

	if(!IS_NPC(victim) && victim->race == RACE_GNOME )
		range = range *	23 / 20;
		...
		if (!IS_NPC(ch ) )
		{
			if (ch->position == POS_FIGHTING )
			{
				if (ch->fighting->who == ch )
					iChance = ch->pcdata->learned[sn]+(get_curr_int(ch ) - (15 +	ch->level /	10 ) );
				else
					iChance = ch->pcdata->learned[sn]+(get_curr_int(ch ) - (10 +	ch->level /	10 ) );
			}
			else
				iChance = ch->pcdata->learned[sn] +	(get_curr_int(ch ) / 2 );
		}

		if (!IS_IMMORTAL(ch ) && !IS_NPC(ch ) && number_percent() > iChance )
		{
...
void spell_acid_blast(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	dam = number_range(level*3+40, level*6+100);
...
void spell_call_lightning(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if (!IS_NPC(ch ) )
		log_printf("%s casting Call Lightning in room %d.", ch->name, ch->in_room->vnum );
...
{
	damage(ch, (CHAR_DATA *) vo, dice(5, 8) + level - 6, sn );
	return;
}
...
void spell_color_spray(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	dam = number_range(level*3+40, level*4+60);
	if (saves_spell(level, ch, victim ) )
		dam /= 2;
...
void spell_curse(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	sprintf(buf, "%s cast curse on %s at room %d", ch->name, victim->name,
		ch->in_room->vnum );
	log_string(buf );
...
void spell_dispel_magic(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	send_to_combat_char("Their magic spells seem to vanish slowly.\n\r", ch );
...
void spell_earthquake(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if (!IS_NPC(ch ) )
		log_printf("%s casting Earthquake in room %d.", ch->name, ch->in_room->vnum );
	...
	for (vch = ch->in_room->first_person; vch != NULL;	vch = vch_next )
	{
		...
		if (vch->in_room == NULL )
			continue;
		if IS_SET(vch->act, PLR_WIZINVIS )
			continue;
...
void spell_tremor(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if (!IS_NPC(ch ) )
		log_printf("%s casting Tremor in room %d.", ch->name, ch->in_room->vnum );
...
void spell_energy_drain(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	dam = number_range(level*2 , level*5) +	5;
	ch->hit += (dam/6);
	victim->move = UMAX(0,number_fuzzy(victim->move-dam*8/13));
...
void spell_faerie_fog(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	for (ich = ch->in_room->first_person; ich != NULL;	ich = ich->next_in_room )
	{
		...
		if (IS_NPC(ch) && IS_AFFECTED(ich, AFF_ETHEREAL) )
			continue;
...
void spell_magic_missile(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	dam = number_range(level/2+2, level+6);
...
void spell_shocking_grasp(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	dam = number_range(level*2+5, level*3+10);
	if (saves_spell(level, ch, victim ) )
		dam /= 2;
	damage(ch, victim, dam, sn );
	return;
...
void spell_summon(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	/* Cops request log of summons - Emp 10/3/99 */

	sprintf(buf, "%s has summoned %s", get_name(ch), victim->name );
	log_string(buf );
...
void spell_weaken(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if (ch != victim )
		send_to_combat_char("They now lost the strength to fight.\n\r", ch);
	return;
}
...
void spell_acid_breath(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	*/
	...
	/* Disabling armor damage until repair is fixed -- Merzik -	10-4-00
...
void spell_gas_breath(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	send_to_combat_char("Your breath fills the room with a deadly gas!\n\r", ch );
	act("$n's breath fills the room with a deadly gas!", ch, NULL, NULL, TO_ROOM );
	...
	{
	ch->in_room = pexit->to_room;
	act("You detect a faint smell of a deadly gas.", ch, NULL, NULL, TO_ROOM );
	}
	}
...
/* Old gas breath code -- Merzik - 10-02-00
...
void spell_block_area(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	sprintf(buf, "%s blocked area in room %d", ch->name, ch->in_room->vnum );
	log_string(buf );
	return;
}
...
void spell_enhance_object(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if(obj->owned_by != 0 || obj->carried_by == NULL )
	{
...
#define HIGHEST_RIFT_VNUM 40000
...
void do_rcast(CHAR_DATA *ch, char *argument )
{
	...
	/* Disallowing rcast from no_mob rooms so mobs do not move to a no_mob room -- Merzik 8/27/2003 */
...
void do_mass(CHAR_DATA *ch, char *argument )
{
	...
	if(cnt==-1 )
		...
		level = ch->mclass[cnt];
	mlev=(IS_NPC(ch))?100:ch->mclass[CLASS_CLERIC];
	...
	if(((skill_table[sn].target==TAR_CHAR_OFFENSIVE	&&
		...
		mlev<skill_table[gsn_mass].skill_level[CLASS_CLERIC])) &&
		!IS_NPC(ch))
	{
...
void spell_illusion(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	add_follower(mh , ch );
	sprintf(buf,"%s cast an illusion named '%s' at room %d.", ch->name, mh->name, ch->in_room->vnum );
	log_string(buf );
	return;
}
...
void spell_benediction(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	af.modifier = number_fuzzy((ch->mclass[CLASS_CLERIC]/6)*25);

	if (victim != ch)
		af.modifier /=2;

	af.duration = number_fuzzy((ch->mclass[CLASS_CLERIC]/15));
	af.bitvector = 0;
	affect_to_char(victim, &af );
...
void spell_righteous_fury(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	af.type = sn;
	af.duration = number_fuzzy(ch->mclass[CLASS_CLERIC]/14);
	af.location = APPLY_HITROLL;
...
void spell_farheal(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if(!IS_NPC(ch) && ch->mclass[CLASS_CLERIC]!=ch->level)
	{
...
void spell_possess(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	log_printf("%s has possessed %s", ch->name, victim->short_descr);
...
void spell_vampiric_touch(int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	{
		dam = number_fuzzy(number_range(level*3, level*6));
...
void do_riftmap(CHAR_DATA *ch, char *argument )
{
	char buf[MAX_INPUT_LENGTH];
	int seed, intValue, intBlock = 0;
	ROOM_INDEX_DATA *to_room;

	intBlock = atol(argument );

	if(intBlock < 1 ||	intBlock > 10 )
	{
		send_to_char("Specify a block value between 1 and 10.\n\r", ch);
		return;
	}

	for(intValue = 0 +	((intBlock - 1 ) * 10); intValue < 10 + ((intBlock	- 1 ) *	10 ); intValue++ )
	{
		seed=intValue;
		seed+=(IS_NPC(ch))?ch->pIndexData->vnum:ch->pcdata->pvnum;
		// seed the random number generator
		...
		// Find a room to connect to...
		for(;((to_room=get_room_index(rand() % 40000))==NULL)||
			IS_SET(to_room->room_flags, ROOM_SAFE)||
			IS_SET(to_room->room_flags, ROOM_IS_CASTLE)||
			IS_SET(to_room->room_flags, ROOM_PET_SHOP) ||
			IS_SET(to_room->room_flags, ROOM_PRIVATE) ||
		...
		sprintf(buf, "%5d %-20s %-40s\n\r", intValue, to_room->area->name, to_room->name );
...
strcat(filename, "_Rifts.txt" );

fpRifts = fopen(filename, "w" );

for(intValue = 0; intValue < 10000; intValue++)
{
seed=intValue;
seed+=(IS_NPC(ch))?ch->pIndexData->vnum:ch->pcdata->pvnum;
srand(seed+1);
...
sprintf(buf, "%d;%s;%s\n", intValue, to_room->area->name, to_room->name );
