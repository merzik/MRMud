...
void advance_level (CHAR_DATA * ch, bool fSave)
{
	...
	/* Makes sure who the player is following is within range -- Merzik 5-14-2001 */
	if (ch->master)
		check_follower(ch, ch->master);
...
void gain_exp (CHAR_DATA * ch, int gain)
{
	...
	{
		ch->exp = exp_level(ch->class, ch->level - 1) + 1;
		return;
	}
...
int hit_gain (CHAR_DATA * ch)
{
	...
	/* Let's making camping actually worth something... -- Merzik */
...
/* Modified to be included in a PHP generated page on the web site -- Merzik */
void save_html_who (void)
{
	...
	fprintf (fp, "<BODY bgcolor=#000000 text=#C0C0C0 link=#9D72C0 vlink=#9D72C0 alink=#9D72C0>\n");

	fprintf (fp, "<HTML><HEAD><TITLE>Mortal Realms Who List</TITLE></HEAD>\n");
	fprintf (fp, "<BODY><FONT SIZE=+2><CENTER>\n");
	*/
	fprintf (fp, "<h3 class=\"pagetitle\" align=\"center\">Players Online</h3>");
	fprintf( fp, "<div align=left>" );
	...
	*/
	...
	for (fpl = first_player; fpl != NULL; fpl = fpl->next)
	{
		...
		*/
		if ((wch->pcdata->switched || wch->desc != NULL) && !IS_AFFECTED (wch, AFF_STEALTH))
		{
			sprintf( buf2, "<td width=\"170\" valign=\"top\">%25s</td></tr>", wch->in_room->area->name );
			/* buf2[20] = '\0';
			while (strlen (buf2) < 20)
			str_cat_max (buf2, " ", MAX_STRING_LENGTH); */
		}
		else
			if (wch->desc == NULL)
				sprintf( buf2, "<td width=\"170\" valign=\"top\">Link Lost</td></tr>" );
			else
		...
		*wch->pcdata->mail_address != '\0')
		...
		/* Nobody seems to care about html addresses -- Merzik
		...
		*pt = '~';
		...
		leng = str_apd_max (buf, "<br><br>\n", leng, MAX_STRING_LENGTH);
		*/
	}

	sprintf (buf2, "</table><p align=\"center\">Players: %d</p>", nTotal);
	...
	if( nTotal < 8 )
		fprintf( fp, "<br><br><br><br><br><br><br><br><br><br>" );
...
char_update (void)
{
	...
	for (iHash = 0; iHash < MAX_KEY_HASH; iHash++)
				...
				if (!IS_NPC (ch))
					if (ch->pcdata->just_died_ctr != 0)
						...
						if (!IS_NPC (ch))
						{
							...
							// Log out players after 15 idle minutes or immortals after 60 minutes -- Merzik 11/8/2004
							if ((ch->timer > 15 && (!IS_IMMORTAL(ch) || !is_desc_valid(ch)))
								|| (ch->timer > 60 && IS_IMMORTAL(ch)))
							{
		...
		*/
		...
		{
			if ( ch_quit->pcdata->switched_char != NULL )
				do_return( ch_quit->pcdata->switched_char, NULL );
...
void update_handler (void)
{
	...
	for (gch = first_player; gch != NULL; gch = gch->next)
		...
		if (--pulse_area <= 0)
		{
			...
			if (now_time.tv_sec > rent_time)/* Collect clan rent and set next rent time */
			{
				struct tm now;
				collect_rent();
