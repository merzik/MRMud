...
*/
#define MAX_EXP_WORTH 2000000
#define MIN_EXP_WORTH 75

#define MAX_SKILL 213
#define MAX_BITVECTOR 562
#define MAX_BODY 14
#define MAX_CLASS 8
#define MAX_RACE 13
#define MAX_LEVEL 99
...
/* File name types used for get_player_filename */
#define PFILE_TYPE_NORMAL 1
#define PFILE_TYPE_DELETE 2
#define PFILE_TYPE_BACKUP 3
#define PFILE_TYPE_TEMP 4
#define PFILE_TYPE_BACKUPTEMP 5
...
#define PLR2_NEWCHAT 8192 /* Newbie channel - Emp */
#define PLR2_CHAOSSAY 16384 /* Config chaos speak- Emp */
#define PLR2_TANK 32768 /* Tank-Teacher for leveling -- Merzik */
...
struct char_data
{
	...
	long critical_hit_by; /* The PVNUM of the hitter */
	sh_int distracted;
	int language;
	int speak;
...
struct pc_data
{
	PC_DATA * next;
	CLAN_DATA * clan;
	CLAN_DATA * clan_listen; /* Pointer used for being a guest on clantalk */
	HELP_DATA * last_help;
	HELP_DATA * prev_help;
	...
	POISON_DATA * poison;
	AREA_DATA * area;
	CHAR_DATA *replylock; /* Used to lock replies to one player */
	int pfile_version;
	char * password;
	char * bamfin;
	char * bamfout;
	...
	int port_size;
	int port_baud;
	long pvnum; /* Player's virtual number */
	...
	CHAR_DATA * switched_char; /* Points to which mob char is switched into for forced return purposes */
	sh_int clan_army; /* Clan army status 0 = Not joined, 1 = Active, 2 = Retired */
	sh_int pkill_type; /* Stores type of pkill command used so we can tally kills correctly */
	sh_int tank_change; /* Whether to allow change in tank status */
};
...
struct clan_data
{
	...
	int pkills[6]; /* Number of kills by clan */
	int pdeaths[6]; /* Number of deaths by player */
	...
	int num_backdoors;
	int num_leaders;
	char * bamfin; /* gohome stuff */
	char * bamfout; /* gohome stuff */
	int bank;
	int type;
};
...
#define HISTORY_KILL_CLAN 7
#define HISTORY_LOSE_CLAN 8
...
#define CLAN_KILL 4
#define CLAN_DEATH 4
#define CLAN_ARMY_KILL 5
#define CLAN_ARMY_DEATH 5
...
struct npc_data
{
	...
	sh_int damplus;
	sh_int armor;
	long pvnum_last_hit; /* For cheating check */
	long pvnum_last_hit_leader; /* For cheating check */
	long mob_number;
...
struct poison_data
{
	...
	long owner; /* Pvnum of creator of poison */
	long poisoner; /* PVNUM of poisoner */
};
...
struct room_index_data
{
	...
	long owned_by; /* The pvnum of the player that owns that room */
...
extern sh_int gsn_counterstrike;
extern sh_int gsn_sweep;
extern sh_int gsn_bodythrow;
...
extern const struct dex_app_type dex_app [40];
...
extern const char ANSI_YELLOW_BOLD[];
extern const char ANSI_GREEN_BOLD[];
extern const char ANSI_CYAN_BOLD[];
extern const char ANSI_BLUE_BOLD[];
extern const char ANSI_RED_BOLD[];
extern const char ANSI_WHITE_BOLD[];
extern const char ANSI_MAGENTA_BOLD[];
extern const char ANSI_BLACK_BOLD[];
extern const char ANSI_YELLOW_DIM[];
extern const char ANSI_GREEN_DIM[];
extern const char ANSI_CYAN_DIM[];
extern const char ANSI_BLUE_DIM[];
extern const char ANSI_RED_DIM[];
extern const char ANSI_WHITE_DIM[];
extern const char ANSI_MAGENTA_DIM[];
extern const char ANSI_BLACK_DIM[];
...
*/
DECLARE_DO_FUN( do_riftmap );
DECLARE_DO_FUN( do_questbits );
DECLARE_DO_FUN( do_advance );
DECLARE_DO_FUN( do_afk );
DECLARE_DO_FUN( do_allow );
DECLARE_DO_FUN( do_allow2 );
...
DECLARE_DO_FUN( do_newchat ); /* added by Emp */
...
DECLARE_DO_FUN( do_clanarmy );
DECLARE_DO_FUN( do_tank );
...
DECLARE_DO_FUN( do_mpmobpurge );
DECLARE_DO_FUN( do_mpstat );
DECLARE_DO_FUN( do_mptransfer );
DECLARE_DO_FUN( do_mpcheatlog );
...
#ifdef CODER_EDITION
#define AREA_LIST "./areas/area.lst"
...
#define SITEBAN_FILE "siteban.lst"
#define SITEBAN_TEMP "siteban.tmp"
#define USAGE_FILE "usage.txt"
#define USAGE_TEMP "usage.tmp"
...
void check_follower args( ( CHAR_DATA *ch, CHAR_DATA *leader ) );
...
bool pvnum_in_group args( ( CHAR_DATA *ch, long pvnum ) );
void do_battlespam args( ( char * argument ) );
...
sh_int get_attack_type args( ( CHAR_DATA *, CHAR_DATA *) );
...
bool can_battle args( ( CHAR_DATA *ch, CHAR_DATA *victim ) );
...
CD * get_char_room_no_illusion args( ( CHAR_DATA *ch, char *argument ) );
...
void make_char_fight_char args( ( CHAR_DATA *ch, CHAR_DATA *victim ) );
...
void save_notes args(());

/* security.c */
bool check_password args((CHAR_DATA *ch, char *szInput));
void set_password args((CHAR_DATA *ch, char *szInput));
void get_MD5_digest args((char *szInput));
char *strMD5Buffer;
...
long next_pvnum; /* Next available pvnum for new characters - initialized via load_config() */

void set_fighting args( ( CHAR_DATA *ch, CHAR_DATA *victim ) );
void reset_color( CHAR_DATA *);

CHAR_DATA *get_pvnum_index( long );
...
#define COLOR_NEWCHAT 16
#define COLOR_CHANNEL_TALK 17
#define COLOR_CLANTALK 18
#define COLOR_MAX 19
...
sh_int Race_Battle_List[MAX_RACE-1];
...
char get_spy_score_txt[MAX_STRING_LENGTH];
